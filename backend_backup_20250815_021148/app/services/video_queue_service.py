...
class VideoQueueService:
            ...
            if self.websocket_manager:
                await self.websocket_manager.send_to_user(user_id, {
                    ...
                    "data": {
                        ...
                        "title": title,
                        "estimated_cost": estimated_cost,
                        "position": await self._get_queue_position(db, queue_item.id)
                    }
                })
    ...
    async def update_processing_status(
        ...
        if self.websocket_manager:
            ...
            if queue_item:
                await self.websocket_manager.send_to_user(queue_item.user_id, {
    ...
    async def get_next_items(
        ...
        db: AsyncSession,
        limit: int = 10,
        worker_type: str = "video_generator"
    ) -> List[VideoQueue]:
        """
    ...
    async def _schedule_celery_task(
        ...
        try:
            ...
            task = celery_app.send_task(
                'app.tasks.video_generation.process_video_queue',
                **task_kwargs
            )
        ...
        result = await db.execute(
            select(func.count(VideoQueue.id)).where(
                and_(
                    VideoQueue.status.in_([QueueStatus.PENDING, QueueStatus.SCHEDULED]),
                    or_(
                        VideoQueue.priority > select(VideoQueue.priority).where(VideoQueue.id == queue_id),
                        and_(
                            VideoQueue.priority == select(VideoQueue.priority).where(VideoQueue.id == queue_id),
                            VideoQueue.created_at < select(VideoQueue.created_at).where(VideoQueue.id == queue_id)
                        )
                    )
                )
            )
    ...
    async def _get_processing_rate(
        self,
        db: AsyncSession,
        user_id: str
    ) -> float:
