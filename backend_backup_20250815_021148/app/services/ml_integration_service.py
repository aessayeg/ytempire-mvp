...
class MLIntegrationService:
    ...
    def _initialize_ml_models(self):
        ...
        try:
            ...
            profiles_path = self.models_path / "personalization" / "channel_profiles.pkl"
    ...
    async def predict_video_performance(
        ...
        """
        if not ML_AVAILABLE or not self.automl_pipeline or not self.automl_pipeline.best_model:
        ...
        try:
            ...
            required_features = [
                'title_length', 'description_length', 'keyword_count',
                'trending_score', 'channel_subscriber_count', 'channel_video_count',
                'posting_hour', 'posting_day', 'video_duration'
            ]
            ...
            return {
                ...
                "confidence_score": model_summary['current_model'].get('validation_score', 0.5),
                "model_type": model_summary['current_model'].get('type', 'unknown'),
                "prediction_factors": {
                    "trending_impact": video_features.get('trending_score', 0) * 0.3,
                    "channel_strength": min(video_features.get('channel_subscriber_count', 0) / 100000, 1.0) * 0.2,
        ...
        try:
            ...
            for video in videos:
                ...
                video_features = {
                    ...
                    'description_length': len(video.description.split()) if video.description else 0,
                    'keyword_count': len(video.keywords) if video.keywords else 0,
                    'trending_score': video.metadata.get('trending_score', 0) if video.metadata else 0,
                    ...
                    'posting_day': video.created_at.weekday() if video.created_at else 0,
        ...
        try:
            ...
            if self.automl_pipeline and self.automl_pipeline.best_model:
                ...
                next_video_features = {
                    ...
                    'keyword_count': 5,
                    'trending_score': 0.7,
                    'channel_subscriber_count': insights.get('performance', {}).get('avg_views', 10000) / 10,
                    'channel_video_count': insights.get('recent_videos', 10),
                    'posting_hour': insights.get('optimal_schedule', {}).get('best_hour', 12),
                    'posting_day': insights.get('optimal_schedule', {}).get('best_day', 4),
                    'video_duration': insights.get('content_preferences', {}).get('preferred_duration', 600)
                }
                
                performance_prediction = await self.predict_video_performance(next_video_features)
    ...
    async def check_retraining_needed(self) -> Dict[str, bool]:
        ...
        if ML_AVAILABLE:
            if self.automl_pipeline:
                results["automl_needs_retraining"] = self.automl_pipeline.should_retrain()
    ...
    def _get_fallback_recommendation(
        ...
        topic = trending_topics[0] if trending_topics else channel_data.get('niche', 'general')
