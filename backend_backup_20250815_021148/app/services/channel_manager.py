...
class ChannelManager:
    ...
    async def get_channel_health(self, channel_id: str) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            return {
                ...
                'status': status,
                'metrics': metrics,
                'recommendations': self._get_health_recommendations(metrics, health_score)
            }
    ...
    async def _calculate_health_metrics(self, db, channel) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'quota_usage': (daily_videos.scalar() or 0) / self.quota_limits['daily_videos'] * 100,
    ...
    def _get_health_recommendations(self, metrics: Dict[str, Any], health_score: int) -> List[str]:
        """Generate health improvement recommendations"""
        recommendations = []
        
        if metrics['error_rate'] > 10:
            recommendations.append("High error rate detected. Review video generation settings.")
        
        if metrics['quota_usage'] > 60:
            recommendations.append("Approaching quota limit. Consider spreading uploads throughout the day.")
        
        if metrics['avg_ctr'] < 2:
            recommendations.append("Low CTR. Improve thumbnails and titles for better engagement.")
        
        if metrics['last_upload'] and (datetime.utcnow() - metrics['last_upload']).days > 3:
            recommendations.append("No recent uploads. Schedule new content to maintain engagement.")
        
        if health_score < 50:
            recommendations.append("Channel health is critical. Immediate attention required.")
    ...
    async def rotate_channels(self, user_id: str) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            return {
                ...
                'active_channel': rotation_order[0]['channel_id'] if rotation_order else None
            }
    ...
    async def optimize_channel_settings(self, channel_id: str) -> Dict[str, Any]:
        ...
        if metrics['error_rate'] > 5:
            ...
            optimizations['daily_limit'] = min(15, self.quota_limits['daily_videos'] + 2)
