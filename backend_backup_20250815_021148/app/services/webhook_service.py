...
class WebhookService:
    ...
    async def deliver_webhook(
        ...
        try:
            # Get delivery record
            result = await db.execute(
                select(WebhookDelivery, Webhook).join(
            ...
            try:
                ...
                if success:
                    ...
                    webhook.last_status_code = response.status_code
                    webhook.last_error_message = f"HTTP {response.status_code}: {response.text[:500]}"
                    
                    # Schedule retry if needed
                    if delivery.attempt_number < delivery.max_attempts:
                        await self._reschedule_delivery(db, delivery, f"HTTP {response.status_code}")
                        
                await db.commit()
                
                logger.info(f"Delivered webhook {delivery.id} with status {response.status_code}")
                return success
    ...
    async def get_webhook_stats(
        ...
        delivery_stats = await db.execute(
            select(
                ...
                func.count(WebhookDelivery.id).filter(WebhookDelivery.success == True).label('successful'),
                func.count(WebhookDelivery.id).filter(WebhookDelivery.success == False).label('failed'),
        ...
        return {
            ...
            "delivery_stats": {
                ...
                "success_rate": (stats_row.successful / stats_row.total * 100) if stats_row.total > 0 else 0,
    ...
    async def _process_event_data(self, event_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _reschedule_delivery(
        ...
        if delivery.attempt_number <= delivery.max_attempts:
            ...
            backoff = delivery.retry_backoff_seconds * (2 ** (delivery.attempt_number - 1))
