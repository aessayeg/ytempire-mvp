...
class VideoGenerationOrchestrator:
    ...
    async def generate_video(
        ...
        try:
            ...
            youtube_account = await youtube_account_manager.get_best_account(quota_needed=2000)
            ...
            await self._update_phase(video_id, GenerationPhase.TREND_ANALYSIS, websocket)
            phase_start = time.time()
            
            if not topic:
                # Detect trending topic
                trending_topics = await self.trend_detector.get_trending_topics(
                    category="technology",  # Can be dynamic based on channel
                    limit=5
                )
            ...
            await self._update_phase(video_id, GenerationPhase.VIDEO_ASSEMBLY, websocket)
            phase_start = time.time()
            ...
            if quality_score["overall_score"] < 70:
                logger.warning(f"Video {video_id} quality score below threshold: {quality_score['overall_score']}")
    ...
    async def _update_phase(self, video_id: str, phase: GenerationPhase, websocket: Optional[WebSocket]):
    ...
    async def _create_video_record(self, video_id: str, channel_id: str, db: AsyncSession) -> Video:
    ...
    async def _update_video_record(self, video: Video, metrics: GenerationMetrics, db: AsyncSession):
        """Update video record with final metrics"""
        video.status = VideoStatus.PUBLISHED if not metrics.errors else VideoStatus.FAILED
        ...
        video.published_at = datetime.utcnow() if video.status == VideoStatus.PUBLISHED else None
    ...
    def get_active_generations(self) -> Dict[str, str]:
        ...
        return {
            video_id: phase.value 
            for video_id, phase in self.active_generations.items()
        }
