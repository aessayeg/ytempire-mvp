...
class VectorDatabaseService:
    ...
    def _init_chroma(self):
        ...
        try:
            self.collections['scripts'] = self.chroma_client.get_collection("video_scripts")
        except:
        ...
        try:
            self.collections['metadata'] = self.chroma_client.get_collection("video_metadata")
        except:
        ...
        try:
            self.collections['trends'] = self.chroma_client.get_collection("trending_topics")
        except:
            ...
            for i in range(len(results['ids'][0])):
                if results['ids'][0][i] not in exclude_ids:
                    recommendations.append({
                        ...
                        'channel': results['metadatas'][0][i].get('channel', 'Unknown'),
            ...
            if self.backend == "chroma":
                ...
                if content:
                    ...
                    if document_id in existing['vectors']:
                        ...
                        existing_vec['metadata']['updated_at'] = datetime.utcnow().isoformat()
...
async def main():
    ...
    results = await vector_db.search(
        collection="scripts",
        query="programming tutorials for beginners",
        top_k=5
    )
