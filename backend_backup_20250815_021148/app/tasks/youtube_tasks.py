    ...
    try:
        ...
        async def upload():
            async with AsyncSessionLocal() as db:
                ...
                youtube_account = await self.youtube_manager.get_best_account_for_upload(
                ...
                if video and upload_result.get('video_id'):
                    ...
                    video.youtube_url = f"https://youtube.com/watch?v={upload_result['video_id']}"
                    video.status = VideoStatus.PUBLISHED if privacy == "public" else VideoStatus.SCHEDULED
                    video.published_at = datetime.utcnow() if privacy == "public" else None
                    await db.commit()
...
@celery_app.task(
    name='youtube.check_upload_status',
    queue='youtube_upload'
)
            ...
            async with AsyncSessionLocal() as db:
                ...
                for yt_video in videos:
                    ...
                    existing = await db.execute(
                        select(Video).where(
                            Video.youtube_video_id == yt_video['id']
                        )
                    )
...
def schedule_video_publish(
    ...
    try:
        ...
        upload_video_to_youtube.apply_async(
            args=[video_id],
            eta=publish_dt
        )
