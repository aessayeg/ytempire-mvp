...
class VideoGenerationRequest(BaseModel):
    ...
    channel_id: str
    topic: str
    style: str = Field(default="educational", description="Video style: educational, entertainment, tech, etc.")
    length: str = Field(default="medium", description="Video length: short (1-3min), medium (3-7min), long (7-15min)")
    target_audience: str = Field(default="general", description="Target audience demographics")
    keywords: Optional[List[str]] = Field(default=None, description="SEO keywords to include")
    voice_provider: str = Field(default="elevenlabs", description="Voice provider: elevenlabs or google_tts")
    ...
    auto_publish: bool = Field(default=False, description="Automatically publish to YouTube")
...
async def get_generation_status(
    ...
    result = await db.execute(
        select(Video).join(Channel).where(
            Video.id == video_id,
            Channel.owner_id == current_user.id
        )
    )
