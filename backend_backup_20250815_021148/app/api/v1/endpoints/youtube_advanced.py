...
class ThumbnailSelectionRequest(BaseModel):
    ...
    video_id: str
    thumbnail_option: str = Field("default", description="Thumbnail option: default, 1, 2, 3, or custom URL")
...
async def select_thumbnail(
    ...
    try:
        ...
        if success:
            return {
                "status": "success",
                "message": "Thumbnail selection completed"
            }
        else:
            return {
                "status": "info",
...
def get_quota_recommendations(percentage_used: float) -> List[str]:
    ...
    if percentage_used < 50:
        recommendations.append("Quota usage is healthy. You can increase API operations if needed.")
    elif percentage_used < 70:
        recommendations.append("Monitor quota usage closely. Consider optimizing API calls.")
    elif percentage_used < 85:
        recommendations.append("Quota usage is high. Implement caching to reduce API calls.")
        recommendations.append("Consider spreading operations throughout the day.")
    else:
        recommendations.append("Quota usage is critical! Reduce non-essential API calls.")
...
def extract_common_words(titles: List[str]) -> List[str]:
    ...
    stop_words = {'the', 'and', 'you', 'for', 'are', 'with', 'this', 'that', 'how', 'not', 'but'}
...
def generate_content_insights(trending_videos: List[Dict]) -> List[str]:
    ...
    if categories:
        ...
        insights.append(f"Most popular category: {top_category} ({categories[top_category]} videos)")
    
    # Upload timing (would need more data in practice)
    insights.append("Consider uploading during peak engagement hours (7-9 PM)")
    
    # Engagement insights
    avg_engagement = sum(v.get("like_count", 0) / max(v.get("view_count", 1), 1) for v in trending_videos) / len(trending_videos)
