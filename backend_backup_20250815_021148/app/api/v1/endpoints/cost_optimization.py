...
class CostEstimateRequest(BaseModel):
    ...
    service: str = Field(..., description="Service type (script_generation, voice_synthesis, etc.)")
...
class OptimalModelRequest(BaseModel):
    ...
    quality_required: float = Field(default=85.0, ge=0, le=100, description="Required quality score")
    budget_remaining: Optional[float] = Field(None, description="Remaining budget for this video")
...
async def get_optimal_model(
    ...
    try:
        ...
        return {
            ...
            "estimated_cost": estimated_cost,
            "quality_required": request.quality_required,
            "meets_requirements": optimizer._estimate_model_quality(model, service) >= request.quality_required
        }
...
async def get_cache_statistics(
    ...
    try:
        ...
        return {
            ...
            "cache_ttl_settings": {
                service.value: ttl 
                for service, ttl in optimizer.CACHE_TTL.items()
            }
        }
