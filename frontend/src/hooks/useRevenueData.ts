...
export const useRevenueData = ({ userId,
  ...
  dateRange,
  period = 'daily',
  breakdownBy = 'source' }: UseRevenueDataProps): UseRevenueDataReturn => {
  ...
  const fetchRevenueData = useCallback(_async () => {
    ...
    try {
      ...
      // Fetch trends
      const trendsResponse = await apiClient.get('/api/v1/revenue/trends', { params: {
          period,
  ...
  channel_id: ch.channel_id,
            ...
            video_count: 0, // Would need additional API call for full details, average_revenue_per_video: 0,
            total_views: 0 }))
        )}
