...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  }, [themeMode, systemTheme]); // eslint-disable-line react-hooks/exhaustive-deps

  const theme = useMemo_(() => {
  ...
  const toggleTheme = useCallback(() => {
    ...
    setStoredTheme(newMode)}, [isDarkMode]); // eslint-disable-line react-hooks/exhaustive-deps

  const setTheme = useCallback((mode: 'light' | 'dark' | 'system') => {
    enableSmoothTransition();
    setThemeMode(mode);
    if (mode !== 'system') {
      setStoredTheme(mode)} else {
  ...
  useEffect(() => {
    ...
    if (metaThemeColor) {
      metaThemeColor.setAttribute('content', isDarkMode ? '#1 a1 d21' : '#ffffff')}
  }, [isDarkMode]);
