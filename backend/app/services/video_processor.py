...
class VideoProcessor:
        ...
        try:
            ...
            if images:
                ...
                await self._create_slideshow(
                    images, video_path, image_duration, resolution, fps, transitions
                )
            else:
                # Create blank video if no images
                video_path = work_dir / "video.mp4"
                await self._create_blank_video(video_path, duration, resolution, fps)

            # Combine video with audio
            final_path = output_path or (work_dir / "final.mp4")
            await self._combine_audio_video(video_path, audio_path, final_path)
    ...
    async def _create_blank_video(
        self, output_path: Path, duration: float, resolution: Tuple[int, int], fps: int
    ):
        ...
        try:
            cmd = [
                ...
                "lavfi",
                "-i",
                f"color=c=black:s={resolution[0]}x{resolution[1]}:d={duration}",
                "-c:v",
                "libx264",
                "-pix_fmt",
                "yuv420p",
    ...
    async def _combine_audio_video(
        self, video_path: Path, audio_path: str, output_path: str
    ):
    ...
    async def generate_thumbnail(
        ...
        try:
            ...
            for line in lines:
                ...
                # Add shadow
                draw.text(
                    (x_position + 2, y_position + 2), line, font=font, fill="black"
                )
                # Main text
            ...
            if not output_path:
                output_path = (
                    self.temp_dir
                    / f"thumbnail_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.jpg"
                )
            else:
    ...
    async def create_video_with_audio(
        ...
        try:
            ...
            if not image_paths:
                logger.info(
                    "No visuals provided, creating video with default background"
                )
            ...
            image_duration = (
                audio_duration / len(image_paths) if image_paths else audio_duration
            )

            # Build FFmpeg filter complex for advanced video assembly
            filter_complex = await self._build_advanced_filter_complex(
                image_paths, image_duration, resolution, title if title else ""
            )
            ...
            if subtitles:
                ...
                final_output = str(
                    Path(output_path).parent / f"final_{Path(output_path).name}"
                )
    ...
    async def _build_ffmpeg_command(
        ...
        cmd.extend(
            [
                ...
                "[out]",
                "-map",
                f"{len(image_paths)}:a",  # Audio is last input
            ]
        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        file_path = (
                            work_dir / f"img_{datetime.utcnow().timestamp()}.{ext}"
                        )
    ...
    async def _create_title_card(
        self, title: str, work_dir: Path, resolution: Tuple[int, int]
    ) -> str:
    ...
    async def _create_srt_file(self, text: str, output_path: Path, duration: float):
