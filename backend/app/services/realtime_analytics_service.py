...
class RealtimeAnalyticsService:
    ...
    async def add_beta_user(self, user_id: str):
        ...
        await self.redis_client.set(
            "beta_users:list", json.dumps(list(self.beta_users))
        )
    ...
    async def _process_metrics_batch(self):
        ...
        await self.redis_client.setex(agg_key, 300, json.dumps(aggregations))

        # Broadcast to live dashboards
        await self._broadcast_to_dashboards(
            {"type": "metrics_update", "data": aggregations}
        )
    ...
    async def _process_events_batch(self):
        ...
        analytics = {
            ...
            "avg_events_per_user": np.mean(list(user_activity.values()))
            if user_activity
            else 0,
            "avg_events_per_session": np.mean(
                [len(events) for events in session_activity.values()]
            )
            if session_activity
            else 0,
            "timestamp": datetime.utcnow().isoformat(),
        ...
        await self.redis_client.setex(analytics_key, 300, json.dumps(analytics))

        # Broadcast to dashboards
        await self._broadcast_to_dashboards(
            {"type": "event_analytics", "data": analytics}
        )
        ...
        try:
            ...
            for event in sorted(events, key=lambda x: x.get("timestamp", "")):
                if event.get("event_type") == "signup" and not signup_time:
                    ...
                    event.get("event_type") == "video_generate" and not first_video_time
                ):
            ...
            if signup_time and first_video_time:
                time_to_first_video = int(
                    (first_video_time - signup_time).total_seconds()
                )

            # Count videos generated today
            videos_today = len(
                [e for e in events if e.get("event_type") == "video_generate"]
            )
            ...
            engagement_count = len(
                [e for e in events if e.get("event_type") in engagement_events]
            )
            ...
            used_features = set(
                e.get("data", {}).get("feature_name")
                for e in events
                if e.get("event_type") == "feature_use"
            )
            feature_adoption_rate = (
                len(used_features & set(available_features)) / len(available_features)
            ) * 100

            # Daily active time (estimate from event frequency)
            if events:
                first_event = min(events, key=lambda x: x.get("timestamp", ""))[
                    "timestamp"
                ]
                last_event = max(events, key=lambda x: x.get("timestamp", ""))[
                    "timestamp"
                ]
                daily_active_time = int(
                    (
                        datetime.fromisoformat(last_event)
                        - datetime.fromisoformat(first_event)
                    ).total_seconds()
                )
            else:
                daily_active_time = 0
    ...
    async def _check_success_criteria(self, metrics: BetaUserMetrics):
        ...
        if (
            metrics.time_to_first_video
            and metrics.time_to_first_video
            > self.success_thresholds["time_to_first_video"]
        ):
            alerts.append(
                {
        ...
        if (
            metrics.videos_generated_today
            < self.success_thresholds["min_videos_per_day"]
        ):
            alerts.append(
                {
                    "type": "low_usage",
        ...
        if alerts:
            ...
            await self.redis_client.lpush(
                alerts_key, *[json.dumps(alert) for alert in alerts]
            )
    ...
    def _calculate_derived_features(
        self, feature_type: str, metrics: List[Dict]
    ) -> Dict[str, Any]:
        ...
        if feature_type == "engagement":
            ...
            if len(values) > 5:
                base_features.update(
                    {
                        "stability": 1 - (np.std(values) / np.mean(values))
                        if np.mean(values) > 0
                        else 0,
                        "efficiency": np.mean(values[-5:]) / np.mean(values[:5])
                        if len(values) >= 10
                        else 1,
                    }
                )
    ...
    async def _get_beta_user_summary(self) -> Dict[str, Any]:
        ...
        active_users = len(
            [m for m in self.beta_metrics_buffer if m.videos_generated_today > 0]
        )
        avg_engagement = np.mean([m.engagement_score for m in self.beta_metrics_buffer])
        high_risk_users = len(
            [m for m in self.beta_metrics_buffer if m.churn_risk_score > 50]
        )
        ...
        return {
            ...
            "success_rate": (active_users / total_users * 100)
            if total_users > 0
            else 0,
        }
        ...
        while self.streaming_active:
            try:
                ...
                if metrics_buffer_size > 5000 or events_buffer_size > 5000:
                    logger.warning(
                        f"High buffer usage - metrics: {metrics_buffer_size}, events: {events_buffer_size}"
                    )
