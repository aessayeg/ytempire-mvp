...
class VideoGenerationPipeline:
    ...
    def __init__(self):
        ...
        self.openai_service = (
            OpenAIService(ai_config) if ai_config.openai_api_key else None
        )
        self.elevenlabs_service = (
            ElevenLabsService(ai_config) if ai_config.elevenlabs_api_key else None
        )
    ...
    async def generate_video(self, config: VideoGenerationConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            if pipeline_result["total_cost"] >= config.max_cost:
                raise Exception(
                    f"Cost limit exceeded: ${pipeline_result['total_cost']:.2f}"
                )
            ...
            metadata_result = await self._generate_metadata(
                config.topic, script_result["script"], config.keywords
            )
            ...
            voice_result = await self._generate_voice(
                script_result["script"]["narration"], config
            )
            ...
            thumbnail_result = await self._generate_thumbnail(
                metadata_result["title"], config.topic, config.thumbnail_style
            )
            ...
            await self._finalize_video_record(
                video_id, pipeline_result, metadata_result, video_result["video_path"]
            )

            pipeline_result["status"] = "completed"
            logger.info(
                f"Video generation completed. Total cost: ${pipeline_result['total_cost']:.2f}"
            )

            # Track total cost
            await self._track_cost(
                config.user_id, video_id, pipeline_result["total_cost"]
            )
        ...
        if not self.openai_service:
            ...
            return {
                "script": {
                    ...
                    "main_points": [
                        {
                            "point": "First amazing fact",
                            "explanation": "Detailed explanation here",
                        },
                        {
                            "point": "Second amazing fact",
                            "explanation": "More details here",
                        },
                        {
                            "point": "Third amazing fact",
                            "explanation": "Even more details",
                        },
                    ],
                    "conclusion": "Thanks for watching! Don't forget to like and subscribe!",
                    "narration": f"Welcome to our channel! Today we're exploring {config.topic}. "
                    * 10,
                },
                "cost": 0.02,
                "tokens_used": 500,
    ...
    async def _generate_metadata(
        self, topic: str, script: Dict, keywords: List[str]
    ) -> Dict[str, Any]:
        ...
        desc_result = await self.openai_service.generate_description(
            title_result["titles"][0], json.dumps(script), keywords
        )
    ...
    async def _generate_voice(
        self, narration_text: str, config: VideoGenerationConfig
    ) -> Dict[str, Any]:
        ...
        try:
            if config.voice_provider == "elevenlabs" and self.elevenlabs_service:
                ...
                result = await self.google_tts_service.text_to_speech(
                    text=narration_text, output_path=str(audio_path)
                )
        except Exception as e:
    ...
    async def _generate_thumbnail(
        self, title: str, topic: str, style: str
    ) -> Dict[str, Any]:
        """Generate video thumbnail"""
        thumbnail_path = await self.thumbnail_generator.generate(
            title=title, topic=topic, style=style
        )

        return {"thumbnail_path": thumbnail_path, "cost": 0.02}  # DALL-E cost estimate

    async def _generate_visuals(
        self, script: Dict, config: VideoGenerationConfig
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            visual_timeline = await stock_footage_service.generate_visual_sequence(
                script_segments, content_type=config.style
            )
            ...
            for segment in visual_timeline:
                for clip in segment.get("clips", []):
                    visual = {
                        ...
                        "path": clip.get(
                            "path", clip.get("url", f"stock_{len(visuals)}.jpg")
                        ),
            ...
            logger.info(
                f"Generated {len(visuals)} visuals using {footage_ratio['method']} method"
            )
            ...
            return {"visuals": visuals, "cost": 0.0}

    async def _assemble_video(
        self, audio_path: str, visuals: List[Dict], script: Dict, metadata: Dict
    ) -> Dict[str, Any]:
    ...
    async def _upload_to_youtube(
        self, video_path: str, thumbnail_path: str, metadata: Dict, channel_id: str
    ) -> Dict[str, Any]:
    ...
    async def _finalize_video_record(
        self, video_id: str, pipeline_result: Dict, metadata: Dict, video_path: str
    ):
        ...
        async with AsyncSessionLocal() as session:
            ...
            if video:
                ...
                video.published_at = (
                    datetime.utcnow() if video.youtube_video_id else None
                )
                await session.commit()
