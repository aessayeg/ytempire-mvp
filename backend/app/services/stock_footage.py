...
class StockFootageService:
    ...
    async def generate_visual_sequence(
        self, script_segments: List[Dict], content_type: str = "general"
    ) -> List[Dict]:
        """
        ...
        for segment in script_segments:
            ...
            footage_clips = await self.find_footage(
                keywords, segment.get("duration", 10)
            )
    ...
    async def find_footage(
        self, keywords: List[str], duration_needed: float = 10
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    video_url, headers=headers, params=params
                ) as response:
                ...
                async with session.get(
                    photo_url, headers=headers, params=params
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                                ...
                                clips.append(
                                    {
                                        ...
                                        "duration": video["videos"]["medium"].get(
                                            "duration", 10
                                        ),
                                        "source": "pixabay",
                                        "id": video["id"],
    ...
    def _get_effect_params(self, effect: str) -> Dict:
        ...
        params = {
            "zoom_in_slow": {"start_scale": 1.0, "end_scale": 1.2, "anchor": "center"},
            "zoom_out_slow": {"start_scale": 1.2, "end_scale": 1.0, "anchor": "center"},
            "pan_left_right": {"start_x": -0.1, "end_x": 0.1, "y": 0},
            "pan_top_bottom": {"x": 0, "start_y": -0.1, "end_y": 0.1},
            "rotate_subtle": {
                "start_rotation": -2,
                "end_rotation": 2,
                "anchor": "center",
