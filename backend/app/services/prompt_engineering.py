...
[EMOTION]: Intended emotional response""",
                ...
                constraints={"max_words": 50, "duration_seconds": 15},
                examples=[],
    ...
    async def generate_prompt(
        self, template_id: str, variables: Dict[str, Any], optimize: bool = True
    ) -> str:
    ...
    async def optimize_prompt(
        self, prompt: str, constraints: Dict[str, Any] = None
        ...
        if constraints and "max_tokens" in constraints:
            ...
            if token_count > max_tokens:
                optimized_prompt = self._truncate_to_token_limit(
        ...
        if constraints and "examples" in constraints and constraints["examples"]:
            optimized_prompt = self._add_few_shot_examples(
                optimized_prompt, constraints["examples"]
            )
    ...
    def _remove_redundancy(self, text: str) -> str:
        ...
        prev_word = ""
        for word in words:
            if word.lower() != prev_word.lower() or word.lower() in [
                "the",
                "a",
                "an",
                "and",
    ...
    def _calculate_optimization_score(
        self, original: str, optimized: str, original_tokens: int, optimized_tokens: int
    ) -> float:
        """Calculate optimization score (0-1)"""
        # Factors: token reduction, readability, structure
        token_score = min(
            1.0, (original_tokens - optimized_tokens) / max(original_tokens, 1)
        )
    ...
    async def auto_improve_template(
        self, template_id: str, performance_data: List[Dict[str, Any]]
    ) -> PromptTemplate:
        ...
        if successful_patterns:
            ...
            template.template = self._incorporate_patterns(
                template.template, common_elements
            )

        # Update performance metrics
        template.performance_metrics["success_rate"] = len(successful_patterns) / len(
            performance_data
        )
        ...
        for template_id, template in self.templates.items():
            ...
            if template.performance_metrics:
                analytics["average_performance"][
                    template_id
                ] = template.performance_metrics.get("success_rate", 0)

        # Sort by performance
        analytics["top_performing"] = sorted(
            analytics["average_performance"].items(), key=lambda x: x[1], reverse=True
        )[:5]
