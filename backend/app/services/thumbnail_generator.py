...
class ThumbnailGenerator:
    ...
    async def generate(
        self, title: str, topic: str, style: str = "modern", use_ai: bool = True
    ) -> str:
        """
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        output_path = (
                            self.temp_dir
                            / f"thumbnail_{datetime.now().timestamp()}.jpg"
                        )
    ...
    def _add_decorative_elements(self, image: Image, style: str, colors: Dict):
        ...
        if style == "tech":
            ...
            # Top-left
            draw.line(
                [(20, 20), (20 + bracket_size, 20)],
                fill=colors["accent"],
                width=bracket_width,
            )
            draw.line(
                [(20, 20), (20, 20 + bracket_size)],
                fill=colors["accent"],
                width=bracket_width,
            )
            # Bottom-right
            draw.line(
                [
                    (self.width - 20 - bracket_size, self.height - 20),
            ...
            draw.line(
                [
                    (self.width - 20, self.height - 20 - bracket_size),
                    (self.width - 20, self.height - 20),
                ],
                fill=colors["accent"],
                width=bracket_width,
