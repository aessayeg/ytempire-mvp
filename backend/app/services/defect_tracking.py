...
class DefectTracker:
    ...
    async def get_defects(
        ...
        if filters:
            ...
            if filters.get("assignee_id"):
                defects = [d for d in defects if d.assignee_id == filters["assignee_id"]]
    ...
    async def update_defect_status(
        ...
        logger.info(f"Updated defect {defect_id}: {old_status.value} → {new_status.value}")
        return defect
    ...
    async def get_defect_statistics(self) -> Dict[str, Any]:
        ...
        for status in DefectStatus:
            status_counts[status.value] = len([d for d in defects if d.status == status])
        ...
        if resolved_defects:
            avg_resolution_time = sum(d.time_to_resolution for d in resolved_defects) / len(resolved_defects)
        
        # Defect escape rate (simplified calculation)
        production_defects = len([d for d in defects if d.source == DefectSource.PRODUCTION])
    ...
    def _calculate_trends(self, defects: List[Defect]) -> Dict[str, Any]:
        ...
        return {
            ...
            "resolved_this_week": len([d for d in defects if d.resolved_at and d.resolved_at >= week_ago]),
            "critical_open": len([d for d in defects if d.severity == DefectSeverity.CRITICAL and d.status not in [DefectStatus.VERIFIED, DefectStatus.CLOSED]])
        }
    ...
    def _deserialize_defect(self, data: Dict) -> Defect:
        ...
        metadata = DefectMetadata(**data.get("metadata", {})) if data.get("metadata") else DefectMetadata()
