...
# Metrics
cache_hits = Counter(
    "cache_hits_total", "Total cache hits", ["cache_tier", "cache_key_type"]
)
cache_misses = Counter(
    "cache_misses_total", "Total cache misses", ["cache_tier", "cache_key_type"]
)
cache_latency = Histogram(
    "cache_operation_latency", "Cache operation latency", ["operation", "cache_tier"]
)
cache_memory_usage = Gauge(
    "cache_memory_usage_bytes", "Cache memory usage", ["cache_tier"]
)
cache_evictions = Counter(
    "cache_evictions_total", "Total cache evictions", ["cache_tier", "reason"]
)
    ...
    async def get(self, key: str, default: Any = None) -> Any:
        ...
        for i, tier in enumerate(self.tiers):
            ...
            if value is not None:
                ...
                cache_latency.labels(operation="get", cache_tier=f"L{i+1}").observe(
                    latency
                )
...
class CacheWarmer:
    ...
    def add_warming_task(self, key: str, loader: Callable, interval: int = 3600):
        ...
        self.warming_tasks.append(
            {"key": key, "loader": loader, "interval": interval, "last_run": None}
        )
    ...
    async def start(self):
        ...
        while self.running:
            for task in self.warming_tasks:
                if task["last_run"] is None or datetime.now() - task[
                    "last_run"
                ] > timedelta(seconds=task["interval"]):
                    await self.cache_manager.warm_cache(
                        task["key"], task["loader"], task["interval"]
                    )
...
class CDNCache:
    ...
    async def _purge_fastly(self, urls: List[str]) -> bool:
        ...
        async with aiohttp.ClientSession() as session:
            for url in urls:
                async with session.post(
                    f"https://api.fastly.com/purge/{url}", headers=headers
                ) as response:
...
async def main():
    ...
    # Set value with tags
    config = CacheConfig(ttl=3600, tags=["user_data", "profile"], compression=True)
