...
class AlertService:
    ...
    async def _send_slack_alert(self, alert_data: Dict):
        ...
        try:
            ...
            slack_message = {
                ...
                "attachments": [{
                    "color": "danger" if alert_data['percentage'] > 100 else "warning",
                    "fields": [
                        ...
                        {"title": "Exceeded By", "value": f"${alert_data['exceeded_by']:.2f}", "short": True},
                        {"title": "Percentage", "value": f"{alert_data['percentage']:.1f}%", "short": True}
                    ],
        ...
        try:
            ...
            discord_message = {
                ...
                "embeds": [{
                    ...
                    "color": 15158332 if alert_data['percentage'] > 100 else 16776960,  # Red or Yellow
                    "fields": [
                        {"name": "Alert Type", "value": alert_data['type'], "inline": True},
                        {"name": "Service", "value": alert_data.get('service', 'N/A'), "inline": True},
                        {"name": "Current Cost", "value": f"${alert_data['current_value']:.2f}", "inline": True},
                        {"name": "Threshold", "value": f"${alert_data['threshold']:.2f}", "inline": True},
                        {"name": "Exceeded By", "value": f"${alert_data['exceeded_by']:.2f}", "inline": True},
                        {"name": "Usage", "value": f"{alert_data['percentage']:.1f}%", "inline": True}
                    ],
    ...
    def _format_email_body(self, alert_data: Dict) -> str:
        ...
        service_info = f"<tr><td><strong>Service:</strong></td><td>{alert_data.get('service', 'N/A')}</td></tr>" if alert_data.get('service') else ""
...
def send_cost_alert_task(alert_data: Dict):
    ...
    asyncio.run(alert_service.send_threshold_alert(
