...
class FeatureExtractor:
    ...
    async def extract_video_features(
        ...
        features["description_word_count"] = len(
            video_data.get("description", "").split()
        )

        # Title features
        title = video_data.get("title", "")
        features["has_numbers"] = int(bool(re.search(r"\d", title)))
        features["has_capitals"] = int(
            sum(1 for c in title if c.isupper()) > len(title) * 0.3
        )
        ...
        features["clickbait_score"] = sum(
            1 for word in clickbait_words if word.lower() in title.lower()
        )
    ...
    async def extract_channel_features(
        ...
        if features["video_count"] > 0:
            features["avg_views_per_video"] = (
                features["total_views"] / features["video_count"]
            )
        else:
        ...
        if features["subscriber_count"] > 0:
            features["engagement_ratio"] = (
                features["total_views"] / features["subscriber_count"]
            )
        else:
    ...
    async def extract_trend_features(
        ...
        if "search_volume" in df.columns:
            ...
            # Trend direction
            if len(df) > 1:
                features["trend_slope"] = np.polyfit(
                    range(len(df)), df["search_volume"], 1
                )[0]
    ...
    async def extract_cost_features(
        ...
        if "created_at" in df.columns:
            ...
            if len(daily_costs) > 1:
                features["cost_trend"] = np.polyfit(
                    range(len(daily_costs)), daily_costs.values, 1
                )[0]
        ...
        if "tokens_used" in df.columns:
            features["total_tokens"] = (
                df["tokens_used"]
                .apply(
                    lambda x: x.get("input", 0) + x.get("output", 0)
    ...
    async def extract_performance_features(
        ...
        if "likes" in df.columns and "views" in df.columns:
            features["like_rate"] = (
                (df["likes"].sum() / df["views"].sum()) if df["views"].sum() > 0 else 0
            )

        if "comments" in df.columns and "views" in df.columns:
            features["comment_rate"] = (
                (df["comments"].sum() / df["views"].sum())
                if df["views"].sum() > 0
                else 0
            )
        ...
        if "estimated_revenue" in df.columns:
            ...
            features["revenue_per_view"] = (
                features["total_revenue"] / features["total_views"]
                if features.get("total_views", 0) > 0
                else 0
            )
    ...
    async def _extract_sentiment_features(
        ...
        features["overall_sentiment"] = (
            features["title_polarity"] + features["desc_polarity"]
        ) / 2

        return features

    async def _extract_growth_features(
        self, channel_data: Dict[str, Any]
    ) -> Dict[str, float]:
        ...
        channel_age_days = (
            datetime.now()
            - pd.to_datetime(channel_data.get("created_at", datetime.now()))
        ).days
...
class FeatureTransformer:
    ...
    def transform(
        self, features_df: pd.DataFrame, scaler_type: str = "standard"
    ) -> np.ndarray:
        """Transform features"""
        df = features_df.copy()
    ...
    def inverse_transform(
        self, transformed_data: np.ndarray, scaler_type: str = "standard"
    ) -> pd.DataFrame:
        ...
        numerical_cols = [
            col for col in self.feature_columns if col not in self.encoders
        ]
        if len(numerical_cols) > 0 and scaler_type in self.scalers:
            df[numerical_cols] = self.scalers[scaler_type].inverse_transform(
                df[numerical_cols]
            )
    ...
    async def save_features(
        ...
        if self.redis_client:
            await self.redis_client.set(
                key, json.dumps(features, default=str), ex=86400  # 24 hour expiry
            )
