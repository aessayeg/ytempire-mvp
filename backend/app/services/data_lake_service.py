...
class DataLakeService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.s3_client = session.client(
                "s3", endpoint_url=settings.S3_ENDPOINT_URL or "http://localhost:9000"
            )
    ...
    async def ingest_data(
        ...
        if partition_by:
            ...
            key = self._generate_partitioned_key(
                dataset_name, partition_keys, object_id, format
            )
        else:
    ...
    async def optimize_storage(self, dataset_name: str) -> Dict[str, Any]:
        ...
        for partition_key, partition_objects in partitions.items():
            if len(partition_objects) > 10:  # Threshold for compaction
                await self._compact_partition(
                    dataset_name, partition_key, partition_objects
                )
        ...
        logger.info(
            f"Optimized {dataset_name}: {results['files_before']} -> {results['files_after']} files"
        )
        return results
    ...
    async def apply_retention_policy(
        ...
        for obj in objects:
            ...
            if last_modified < cutoff_date:
                ...
                await self._transition_storage_tier(obj["Key"], archive_tier)
                archived_count += 1
                    ...
                    for obj in page["Contents"]:
                        ...
                        if key_parts:
                            ...
                            if dataset not in metrics["by_dataset"]:
                                metrics["by_dataset"][dataset] = {"count": 0, "size": 0}
                            metrics["by_dataset"][dataset]["count"] += 1
                            metrics["by_dataset"][dataset]["size"] += obj["Size"]

                        # Track oldest/newest
                        if (
                            not metrics["oldest_object"]
                            or obj["LastModified"] < metrics["oldest_object"]
                        ):
                            metrics["oldest_object"] = obj["LastModified"]
                        if (
                            not metrics["newest_object"]
                            or obj["LastModified"] > metrics["newest_object"]
                        ):
        ...
        while True:
            ...
            for key in keys:
                ...
                if obj_data:
                    ...
                    metrics["by_format"][format_type]["size"] += obj.get(
                        "size_bytes", 0
                    )
            ...
            try:
                ...
                await s3.create_bucket(
                    ...
                    CreateBucketConfiguration={
                        "LocationConstraint": settings.S3_REGION or "us-east-1"
                    },
                )
    ...
    async def _initialize_catalog(self):
        ...
        await self.redis_client.set(catalog_key, json.dumps(catalog_metadata))

    async def _convert_to_format(self, data: pd.DataFrame, format: DataFormat) -> bytes:
    ...
    async def _read_object(self, key: str, format: DataFormat) -> pd.DataFrame:
        """Read object from S3"""
        async with self.s3_client as s3:
    ...
    async def _list_objects_with_metadata(self, prefix: str) -> List[Dict[str, Any]]:
    ...
    async def _compact_partition(
        self, dataset_name: str, partition_key: str, objects: List[Dict[str, Any]]
    ):
        ...
        async with self.s3_client as s3:
            await s3.put_object(Bucket=self.bucket_name, Key=new_key, Body=data_bytes)

            # Delete original files
            for obj in objects:
                await s3.delete_object(Bucket=self.bucket_name, Key=obj["Key"])

    async def _transition_storage_tier(self, key: str, new_tier: StorageTier):
    ...
    def _extract_partition_keys(
        self, data: pd.DataFrame, partition_by: List[str]
    ) -> Dict[str, str]:
        ...
        for col in partition_by:
            if col in data.columns:
                ...
                if "timestamp" in data.columns or "created_at" in data.columns:
                    time_col = (
                        "timestamp" if "timestamp" in data.columns else "created_at"
                    )
    ...
    def _build_query_prefix(
        self, dataset_name: str, partition_filters: Optional[Dict[str, Any]]
    ) -> str:
        """Build S3 prefix for querying"""
        prefix = f"{dataset_name}/"

        if partition_filters:
            partition_path = "/".join(f"{k}={v}" for k, v in partition_filters.items())
    ...
    async def _store_object_metadata(self, obj: DataLakeObject):
        ...
        await self.redis_client.setex(
            key, 86400 * 90, json.dumps(asdict(obj), default=str)  # 90 days retention
        )

    async def _store_catalog_entry(self, entry: DataCatalogEntry):
        """Store catalog entry"""
        key = f"catalog:table:{entry.table_name}"
        await self.redis_client.set(key, json.dumps(asdict(entry), default=str))

    async def _get_catalog_entry(self, dataset_name: str) -> Optional[DataCatalogEntry]:
    ...
    async def _update_catalog(
        ...
        if not entry:
            ...
            # Update statistics
            entry.statistics["row_count"] = entry.statistics.get("row_count", 0) + len(
                data
            )
            entry.statistics["size_bytes"] = (
                entry.statistics.get("size_bytes", 0)
    ...
    async def _create_hive_metadata(self, entry: DataCatalogEntry):
        ...
        hive_metadata = {
            ...
            "columns": [
                {"name": col, "type": dtype} for col, dtype in entry.schema.items()
            ],
            "partitionKeys": entry.partition_columns,
        }

        key = f"hive:table:{entry.table_name}"
        await self.redis_client.set(key, json.dumps(hive_metadata))
    ...
    async def _apply_lifecycle_policy(self, obj: DataLakeObject):
        ...
        if obj.storage_tier == StorageTier.HOT:
            ...
            await self.redis_client.zadd(
                "lifecycle:tasks", {json.dumps(task): transition_date.timestamp()}
            )
