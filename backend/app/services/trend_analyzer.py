...
try:
    from ml_pipeline.src.trend_detection_model import TrendDetectionModel, TrendingTopic, TrendSource
...
class TrendAnalyzer:
        ...
        if ML_MODELS_AVAILABLE:
            try:
                self.ml_model = TrendDetectionModel(
                    model_path=settings.ML_MODEL_PATH if hasattr(settings, 'ML_MODEL_PATH') else "models/trend_detection"
                )
            ...
            if self.ml_model:
                ...
                for trend in trends:
                    result.append({
                        ...
                        'best_time_to_post': trend.best_time_to_post.isoformat() if trend.best_time_to_post else None,
        ...
        try:
            if self.ml_model:
                ...
                return {
                    ...
                    'recommended_publish_time': (datetime.now() + timedelta(hours=4)).isoformat(),
    ...
    async def _generate_mock_trends(self, category: str, limit: int) -> List[Dict[str, Any]]:
        """Generate mock trend data for testing/fallback"""
        mock_topics = {
            TrendCategory.TECHNOLOGY: [
                "AI Agents", "Quantum Computing", "Web3", "Robotics", "Cybersecurity",
                "5G Networks", "Blockchain", "IoT", "AR/VR", "Cloud Computing"
            ],
            TrendCategory.GAMING: [
                "Baldur's Gate 3", "Starfield", "CS2", "Minecraft Updates", "Steam Deck",
                "PlayStation 5", "Xbox Series X", "Nintendo Switch", "VR Gaming", "Esports"
            ],
            TrendCategory.EDUCATION: [
                "Online Learning", "AI Tutors", "STEM Education", "Coding Bootcamps",
                "Language Learning", "Digital Skills", "Remote Education", "EdTech", 
                "Microlearning", "Skill Development"
            ]
        }
        
        topics = mock_topics.get(category, mock_topics[TrendCategory.TECHNOLOGY])[:limit]
        ...
        for i, topic in enumerate(topics):
            trends.append({
                ...
                'best_time_to_post': (datetime.now() + timedelta(hours=i+1)).isoformat(),
                'source_scores': {
                    'youtube': 90 - i * 3,
    ...
    def _generate_mock_keywords(self, topic: str, limit: int) -> List[str]:
        ...
        # Add related terms
        suffixes = ['tutorial', 'guide', 'tips', 'tricks', 'review', 'news', 
                   'update', '2024', 'best', 'how to', 'explained', 'vs']
