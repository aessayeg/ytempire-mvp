...
from app.models.payments import (
    PaymentCustomer, Subscription, Payment, Invoice, 
    PaymentMethod, UsageRecord, BillingAlert
)
...
class EnhancedPaymentService:
    ...
    async def track_usage(
        ...
        try:
            ...
            unit_price = await self._get_usage_unit_price(subscription.plan_tier, usage_type)
    ...
    async def _create_subscription_record(
        ...
        subscription = Subscription(
            ...
            billing_cycle='monthly' if price.recurring.interval == 'month' else 'yearly',
    ...
    async def _check_usage_limits(self, db: AsyncSession, subscription: Subscription):
        ...
        # Check video limit
        if subscription.videos_generated_current_period >= subscription.video_limit_monthly * 0.8:
        ...
        if subscription.api_calls_current_period >= subscription.api_calls_limit_monthly * 0.8:
            await self._create_usage_alert(
                db=db,
                user_id=subscription.user_id,
    ...
    # Webhook handlers
    async def _handle_checkout_completed(self, db: AsyncSession, session: Dict[str, Any]):
    ...
    async def _handle_subscription_created(self, db: AsyncSession, subscription: Dict[str, Any]):
    ...
    async def _handle_subscription_updated(self, db: AsyncSession, subscription: Dict[str, Any]):
    ...
    async def _handle_subscription_deleted(self, db: AsyncSession, subscription: Dict[str, Any]):
    ...
    async def _handle_payment_succeeded(self, db: AsyncSession, invoice: Dict[str, Any]):
    ...
    async def _handle_trial_will_end(self, db: AsyncSession, subscription: Dict[str, Any]):
