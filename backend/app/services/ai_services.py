...
class OpenAIService:
    ...
    async def generate_script(
        ...
        try:
            ...
            prompt = self._build_script_prompt(
                topic, style, length, target_audience, keywords
            )
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional YouTube script writer who creates engaging, informative, and well-structured video scripts.",
                    },
        ...
        try:
            ...
            return {
                ...
                "cost": self._calculate_cost(
                    response.usage.total_tokens, "gpt-3.5-turbo"
                ),
            }
    ...
    async def generate_description(
        self, title: str, script: str, keywords: List[str] = None
    ) -> Dict[str, Any]:
    ...
    async def generate_thumbnail_prompt(
        self, title: str, style: str = "vibrant"
    ) -> str:
...
class ElevenLabsService:
    ...
    async def clone_voice(
        self, name: str, audio_files: List[str], description: Optional[str] = None
    ) -> Dict[str, Any]:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                for audio_file in audio_files:
                    with open(audio_file, "rb") as f:
                        data.add_field(
                            "files", f, filename=os.path.basename(audio_file)
                        )
    ...
    async def text_to_speech(
        ...
        try:
            ...
            audio_config = texttospeech.AudioConfig(
                audio_encoding=audio_config_map.get(
    ...
    async def generate_complete_video_content(
        ...
        try:
            ...
            description_result = await self.openai.generate_description(
                title=title_result["titles"][0], script=script_text
            )
            ...
            if voice_service == "elevenlabs":
                audio_result = await self.elevenlabs.text_to_speech(
                    text=narration_text, voice_id=voice_id
                )
            else:  # google_tts
                audio_result = await self.google_tts.text_to_speech(text=narration_text)
            ...
            thumbnail_prompt = await self.openai.generate_thumbnail_prompt(
                title=title_result["titles"][0], style="vibrant"
            )
