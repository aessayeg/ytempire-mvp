...
class AdaptiveCache:
    ...
    def _calculate_adaptive_ttl(self, key: str) -> int:
        ...
        recent_accesses = [
            a for a in accesses if a > datetime.utcnow() - timedelta(hours=1)
        ]
...
class ScalingOptimizer:
    ...
    def _initialize_semaphores(self):
        ...
        self.processing_semaphores = {
            ...
            "video_processing": asyncio.Semaphore(
                max(1, self.current_config.async_workers // 4)
            ),
            "ai_services": asyncio.Semaphore(
                max(1, self.current_config.async_workers // 2)
            ),
    ...
    async def _system_monitor(self):
        ...
        while self.processing_active:
            try:
                ...
                # Get queue depths
                queue_depths = {
                    name: queue.qsize() for name, queue in self.priority_queues.items()
                }
    ...
    def _determine_scaling_mode(self) -> ScalingMode:
        ...
        if (
            self.current_metrics.cpu_usage > 95
            or self.current_metrics.memory_usage > 95
            or sum(self.current_metrics.queue_depths.values()) > 50000
        ):
            return ScalingMode.EMERGENCY

        # Check for peak load
        if (
            self.current_metrics.cpu_usage > 85
            or self.current_metrics.memory_usage > 85
            or sum(self.current_metrics.queue_depths.values()) > 20000
        ):
            return ScalingMode.PEAK_LOAD

        # Check for high load
        if (
            self.current_metrics.cpu_usage > 70
            or self.current_metrics.memory_usage > 70
            or sum(self.current_metrics.queue_depths.values()) > 5000
        ):
    ...
    async def _switch_scaling_mode(self, new_mode: ScalingMode):
        ...
        logger.info(
            f"Switching scaling mode: {self.current_mode.value} -> {new_mode.value}"
        )
    ...
    async def _reconfigure_for_scaling(
        self, old_config: ScalingConfiguration, new_config: ScalingConfiguration
    ):
        ...
        try:
            ...
            logger.info(
                f"Reconfigured for {new_config.mode.value}: "
    ...
    async def _process_task_batch(self, tasks: List[Any], priority: str):
        ...
        try:
            ...
            async with semaphore:
                ...
                timeout = (
                    30 if priority == "high" else 60 if priority == "normal" else 120
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Batch processing timeout for {priority} priority tasks"
                    )
    ...
    async def _apply_performance_optimizations(self):
        ...
        if self.current_config.batch_size > 20:
            self.current_config.batch_size = max(
                20, self.current_config.batch_size // 2
            )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_score": np.mean(list(self.performance_scores)[-10:])
