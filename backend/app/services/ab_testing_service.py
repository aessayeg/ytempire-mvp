...
class ABTestingService:
    ...
    async def start_experiment(
        self, db: AsyncSession, experiment_id: int
    ) -> Dict[str, Any]:
        """Start an experiment"""
        from app.models.experiment import Experiment
    ...
    async def get_variant_assignment(
        self, db: AsyncSession, experiment_id: int, user_id: int
    ) -> Dict[str, Any]:
        ...
        if assignment:
            result = {
                ...
                "config": self._get_variant_config(
                    experiment.variants, assignment.variant_name
                ),
            }
        else:
            # Assign variant
    ...
    async def get_experiment_results(
        ...
        results = {
            ...
            "can_conclude": all(
                m.sample_size >= required_sample_size for m in variant_metrics
            ),
        }
    ...
    async def conclude_experiment(
        self, db: AsyncSession, experiment_id: int, winner_variant: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def _assign_variant(
        self, user_id: int, variants: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _get_variant_config(
        self, variants: List[Dict[str, Any]], variant_name: str
    ) -> Dict[str, Any]:
    ...
    def _calculate_confidence_interval(
        ...
        spread = (
            z
            * np.sqrt((p_hat * (1 - p_hat) + z**2 / (4 * trials)) / trials)
            / denominator
        )
    ...
    def _determine_winner(self, metrics: List[ExperimentMetrics]) -> Optional[str]:
