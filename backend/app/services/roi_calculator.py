...
class ROICalculatorService:
    ...
    async def calculate_overall_roi(
        ...
        result = {
            ...
            "summary": {
                ...
                "cost_revenue_ratio": round((total_cost / total_revenue) if total_revenue > 0 else 0, 3)
            },
    ...
    async def calculate_channel_roi(
        ...
        channel_roi = ChannelROI(
            ...
            avg_video_roi=np.mean([v.roi_percentage for v in video_rois]) if video_rois else 0,
            best_video_roi=max([v.roi_percentage for v in video_rois]) if video_rois else 0,
            worst_video_roi=min([v.roi_percentage for v in video_rois]) if video_rois else 0,
            videos_count=len(videos),
            profitable_videos=profitable_videos,
            break_even_days=self._calculate_break_even_days(video_rois),
            payback_period=self._calculate_payback_period(total_cost, total_revenue, period_days),
            lifetime_value=await self._estimate_lifetime_value(db, channel_id, total_revenue, period_days)
        )
        
        return channel_roi
        
    async def calculate_video_roi(
        self,
        db: AsyncSession,
        video_id: str
    ) -> VideoROI:
        ...
        if cost_per_view > 0:
            break_even_views = int(total_cost / revenue_per_view) if revenue_per_view > 0 else 0
        else:
            break_even_views = 0
    ...
    def _estimate_video_revenue(
        ...
        engagement_multiplier = 1 + min(engagement_rate * 10, 2)  # Up to 3x for high engagement
    ...
    async def _generate_roi_insights(
        ...
        if roi_metrics["predicted_roi"] > roi_metrics["basic_roi"] * 1.2:
            insights["opportunities"].append({
                ...
                "potential_gain": roi_metrics["predicted_roi"] - roi_metrics["basic_roi"]
            })
