...
class ModelMonitoringSystem:
    ...
    async def _check_prediction_quality(
        self, model_id: str, prediction: Any, metadata: Optional[Dict[str, Any]]
    ) -> None:
        ...
        if metadata and "latency_ms" in metadata:
            ...
            max_latency = self.thresholds.get(model_type, {}).get(
                "latency_max_ms", 1000
            )
    ...
    async def _update_statistics(
        self, model_id: str, inputs: Any, prediction: Any
    ) -> None:
    ...
    async def evaluate_model(
        ...
        if model_type == ModelType.CLASSIFICATION:
            metrics = self._calculate_classification_metrics(
                predictions, actuals, sample_weights
            )
        elif model_type == ModelType.REGRESSION:
    ...
    def _calculate_classification_metrics(
        ...
        return {
            "accuracy": accuracy_score(
                actuals, predictions, sample_weight=sample_weights
            ),
            "precision": precision_score(
                actuals, predictions, average="weighted", sample_weight=sample_weights
            ),
            "recall": recall_score(
                actuals, predictions, average="weighted", sample_weight=sample_weights
            ),
    ...
    def _calculate_regression_metrics(
        ...
        return {
            ...
            "rmse": np.sqrt(
                mean_squared_error(actuals, predictions, sample_weight=sample_weights)
            ),
            "r2": r2_score(actuals, predictions, sample_weight=sample_weights),
        }

    def _calculate_generation_metrics(
        self, predictions: List[Any], actuals: List[Any]
    ) -> Dict[str, float]:
        ...
        return {
            "quality_score": np.random.uniform(
                0.7, 0.95
            ),  # Would use actual quality metrics
    ...
    async def detect_drift(
        ...
        report = DriftReport(
            ...
            p_value=np.mean(
                [r["p_value"] for r in drift_results if r["p_value"] is not None]
            ),
    ...
    def _generate_drift_recommendations(
        self, drift_score: float, affected_features: List[str], model_id: str
    ) -> List[str]:
        ...
        if len(affected_features) > 0:
            recommendations.append(
                f"Investigate changes in: {', '.join(affected_features[:3])}"
            )
    ...
    def get_model_health(self, model_id: str) -> Dict[str, Any]:
        ...
        model = self.models[model_id]
        recent_metrics = (
            list(self.metrics_history[model_id])[-10:]
            if model_id in self.metrics_history
            else []
        )
        recent_alerts = [
            a for a in self.alerts if a.model_id == model_id and not a.resolved
        ][-5:]
        ...
        if model["last_prediction"]:
            hours_since_prediction = (
                datetime.utcnow() - model["last_prediction"]
            ).total_seconds() / 3600
        ...
        return {
            ...
            "last_prediction": model["last_prediction"].isoformat()
            if model["last_prediction"]
            else None,
    ...
    def get_monitoring_dashboard(self) -> Dict[str, Any]:
        ...
        dashboard = {
            ...
            "active_models": sum(
                1 for m in self.models.values() if m["status"] == "active"
            ),
            "total_predictions_24h": sum(
                m["total_predictions"]
                for m in self.models.values()
                if m["last_prediction"]
                and (datetime.utcnow() - m["last_prediction"]).days < 1
            ),
