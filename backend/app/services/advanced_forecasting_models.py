...
class AdvancedForecastingModels:
    ...
    async def _forecast_arima(
        self, data: pd.DataFrame, config: ForecastConfig
    ) -> ForecastResult:
        """ARIMA forecasting"""
    ...
    async def _forecast_exponential_smoothing(
        ...
        try:
            ...
            # Calculate accuracy metrics
            accuracy_metrics = self._calculate_accuracy_metrics(
                data.iloc[:, 0].values, fitted_model.fittedvalues
            )
            ...
            return ForecastResult(
                ...
                model_params={
                    ...
                    "smoothing_seasonal": fitted_model.params.get(
    ...
    async def _forecast_random_forest(
        ...
        try:
            ...
            predictions_df = pd.DataFrame(
                {"forecast": predictions, "date": future_dates}
            ).set_index("date")

            # Calculate confidence intervals using prediction intervals from trees
            tree_predictions = np.array(
                [tree.predict(future_X) for tree in model.estimators_]
            )
            lower = np.percentile(
                tree_predictions, (1 - config.confidence_level) / 2 * 100, axis=0
            )
            upper = np.percentile(
                tree_predictions, (1 + config.confidence_level) / 2 * 100, axis=0
            )
            ...
            confidence_intervals = pd.DataFrame(
                {"lower": lower, "upper": upper}, index=predictions_df.index
            )
    ...
    async def _forecast_gradient_boosting(
        self, data: pd.DataFrame, config: ForecastConfig
    ) -> ForecastResult:
    ...
    async def _forecast_ensemble(
        ...
        # Create predictions dataframe
        future_dates = pd.date_range(
            start=data.index[-1] + timedelta(days=1), periods=config.horizon, freq="D"
        )

        predictions_df = pd.DataFrame(
            {"forecast": ensemble_predictions, "date": future_dates}
        ).set_index("date")
        ...
        return ForecastResult(
            ...
            model_params={
                ...
                "models": [m.value for m in models_to_use],
            },
        )

    async def _forecast_baseline(
        self, data: pd.DataFrame, config: ForecastConfig
    ) -> ForecastResult:
    ...
    def _select_arima_params(self, data: pd.DataFrame) -> Tuple[int, int, int]:
        ...
        try:
            ...
            if len(data) > 40:
                ...
                p = (
                    np.where(np.abs(pacf_values) < 0.1)[0][0]
                    if np.any(np.abs(pacf_values) < 0.1)
                    else 1
                )
                q = (
                    np.where(np.abs(acf_values) < 0.1)[0][0]
                    if np.any(np.abs(acf_values) < 0.1)
                    else 1
                )
    ...
    def _create_time_features(
        self, data: pd.DataFrame, lags: int = 7
    ...
    def _create_future_features(
        self, data: pd.DataFrame, horizon: int, lags: int = 7
    ) -> np.ndarray:
    ...
    def _calculate_accuracy_metrics(
        ...
        metrics["mse"] = (
            ...
            else np.mean((actual - predicted) ** 2)
        )
        ...
        metrics["mae"] = (
            ...
            if SKLEARN_AVAILABLE
            else np.mean(np.abs(actual - predicted))
        )
    ...
    def _cache_forecast(self, cache_key: str, result: ForecastResult):
        ...
        self.forecast_cache = {
            k: v for k, v in self.forecast_cache.items() if v[1] > cutoff_time
        }
    ...
    async def compare_models(
        ...
        for model_type in models:
            try:
                config = ForecastConfig(
                    model_type=model_type, metric=metric, horizon=horizon
                )
