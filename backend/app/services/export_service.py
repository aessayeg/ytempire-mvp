...
class DataExportService:
    ...
    async def process_export(self, job_id: str):
        ...
        try:
            ...
            async with AsyncSession() as db:
                ...
                logger.info(
                    f"Completed export job {job_id}: {row_count} rows, {file_size} bytes"
                )
    ...
    async def _fetch_data(
        self, db: AsyncSession, data_type: DataType, parameters: Dict[str, Any]
    ) -> pd.DataFrame:
        """Fetch data based on type"""
        ...
        if data_type == DataType.ANALYTICS:
            return await self._fetch_analytics_data(
                db, start_date, end_date, channel_id
            )
            ...
            return await self._fetch_performance_data(
                db, start_date, end_date, channel_id
            )
    ...
    async def _execute_custom_query(self, db: AsyncSession, query: str) -> pd.DataFrame:
    ...
    async def _export_data(
        self, data: pd.DataFrame, format: ExportFormat, job_id: str
    ) -> Tuple[Path, int, int]:
        """Export data to file"""
    ...
    async def _fetch_data_stream(
        ...
        while True:
            ...
            chunk = await self._fetch_data_chunk(
                db, data_type, parameters, offset, self.chunk_size
            )
    ...
    async def _format_chunk(self, chunk: pd.DataFrame, format: ExportFormat) -> bytes:
        """Format a data chunk"""
        buffer = io.BytesIO()
    ...
    async def list_user_exports(
        self, user_id: str, status: Optional[ExportStatus] = None
    ) -> List[ExportJob]:
    ...
    async def _store_job(self, job: ExportJob):
        ...
        await self.redis_client.setex(
            key, self.export_ttl, json.dumps(asdict(job), default=str)
        )

    async def _store_schedule(self, schedule: ExportSchedule):
        """Store schedule metadata"""
        key = f"export_schedule:{schedule.schedule_id}"
        await self.redis_client.set(key, json.dumps(asdict(schedule), default=str))
