...
class GPUResourceManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"GPU Resource Manager initialized with {len(self.devices)} devices"
            )
            ...
            for i in range(device_count):
                ...
                try:
                    ...
                    for proc in process_info:
                        processes.append(
                            {"pid": proc.pid, "memory_used": proc.usedGpuMemory}
                        )
                except:
                    pass
                ...
                device = GPUDevice(
                    ...
                    if utilization.gpu < 50
                    else GPUStatus.BUSY,
                )

                self.devices[i] = device
                logger.info(
                    f"Discovered GPU {i}: {name} ({memory_info.free / 1024**3:.1f}GB free)"
                )
    ...
    async def allocate_gpu(
        self, task: GPUTask, prefer_device: Optional[int] = None
    ) -> Optional[int]:
        """
        ...
        for device_id, device in self.devices.items():
            if device.status in [GPUStatus.AVAILABLE, GPUStatus.BUSY]:
                ...
                available_memory = device.memory_free - (
                    self.memory_reserve_mb * 1024 * 1024
                )
        ...
        if prefer_device is not None and prefer_device in [
            d[0] for d in suitable_devices
        ]:
        ...
        device.reserved_until = datetime.utcnow() + timedelta(
    ...
    async def queue_task(self, task: GPUTask) -> str:
        ...
        logger.info(
            f"Queued GPU task {task.task_id} with priority {task.priority.name}"
        )
        return task.task_id
        ...
        for task_id, task in self.active_tasks.items():
            if task.started_at:
                ...
                if elapsed > task.estimated_duration * 2:
                    logger.warning(
                        f"Task {task_id} appears stuck (elapsed: {elapsed}s)"
                    )
    ...
    async def store_metrics(self):
        ...
        metrics = {"timestamp": datetime.utcnow().isoformat(), "devices": {}}
    ...
    async def store_task_in_redis(self, task: GPUTask):
        ...
        await self.redis_client.setex(
            f"gpu:task:{task.task_id}", 86400, json.dumps(task_data)  # 24 hours
        )
