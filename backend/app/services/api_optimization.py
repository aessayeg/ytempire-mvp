...
class APIOptimizer:
    ...
    async def make_optimized_request(
        ...
        try:
            ...
            if self._is_circuit_breaker_open(config.provider):
                if config.fallback_providers:
                    return await self._try_fallback_providers(
                        config, request_data, headers, user_id
                    )
                else:
            ...
            self._record_metrics(
                config, start_time, 200, config.cost_per_request, False, 0
            )
            ...
            if config.cost_per_request > 0 and user_id:
                await cost_tracker.track_api_call(
                    ...
                    metadata={"response_time_ms": response_time, "cache_hit": False},
                )
    ...
    async def _check_cache(
        self, config: APIEndpointConfig, request_data: Dict[str, Any]
    ...
    async def _cache_result(
        ...
        try:
            ...
            await redis_client.setex(
                cache_key, config.cache_ttl_seconds, json.dumps(result)
            )
    ...
    async def _check_rate_limit(
        self, config: APIEndpointConfig, user_id: Optional[str]
    ) -> bool:
    ...
    async def _make_request(
        ...
        request_headers.update(
            {"Content-Type": "application/json", "User-Agent": "YTEmpire-API/1.0"}
        )
    ...
    async def _try_fallback_providers(
        ...
        for fallback_provider in primary_config.fallback_providers:
            try:
                ...
                logger.warning(
                    f"Fallback provider {fallback_provider.value} failed: {e}"
                )
                continue
    ...
    async def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            / len(recent_metrics)
            * 100,
            "average_response_time": sum(m.response_time_ms for m in recent_metrics)
            / len(recent_metrics),
            ...
            if provider_metrics:
                provider_stats[provider.value] = {
                    ...
                    "avg_response_time": sum(
                        m.response_time_ms for m in provider_metrics
                    )
                    / len(provider_metrics),
                    "cache_hit_rate": sum(1 for m in provider_metrics if m.cache_hit)
                    / len(provider_metrics)
                    * 100,
                    "error_rate": sum(
                        1 for m in provider_metrics if m.status_code >= 400
                    )
                    / len(provider_metrics)
                    * 100,
                }
    ...
    async def clear_cache(self, provider: Optional[APIProvider] = None):
        ...
        try:
            ...
            if keys:
                ...
                logger.info(
                    f"Cleared {len(keys)} cache entries for {provider.value if provider else 'all providers'}"
                )
    ...
    async def update_rate_limits(
        self, provider: APIProvider, per_minute: int, per_hour: int
    ):
        ...
        logger.info(
            f"Updated rate limits for {provider.value}: {per_minute}/min, {per_hour}/hour"
        )
