...
class FeatureStore:
    ...
    def __init__(
        self, redis_client: redis.Redis = None, db_session: AsyncSession = None
    ):
                    ...
                    if value:
                        ...
                        if value is not None:
                            ...
                            await self._cache_feature(
                                entity_id, feature_name, value, version
                            )
    ...
    async def get_offline_features(
        ...
        result = await self.db_session.execute(
            query,
            {"entity_ids": entity_ids, "start_date": start_date, "end_date": end_date},
        )
            ...
            if feature_name in self.features:
                ...
                if feature_def.transformation:
                    df[feature_name] = await self._apply_transformation(
                        df[feature_name], feature_def.transformation
                    )
    ...
    async def _compute_feature(self, entity_id: str, feature_name: str) -> Any:
        ...
        if feature_def.dependencies:
            ...
            for dep in feature_def.dependencies:
                dep_value = await self.get_online_features([entity_id], [dep])
        ...
        if feature_def.source == FeatureSource.COMPUTED:
            ...
            if feature_def.transformation in self.transformations:
                ...
                return transform_func(
                    entity_id, dep_values if feature_def.dependencies else None
                )
    ...
    async def _cache_feature(
        ...
        await self.redis_client.setex(
            cache_key, feature_def.ttl_seconds, json.dumps(value, default=str)
        )

    def _get_cache_key(
        self, entity_id: str, feature_name: str, version: Optional[str] = None
    ) -> str:
    ...
    def _validate_feature(self, feature: FeatureDefinition) -> bool:
        ...
        if feature.dependencies:
            ...
            if has_cycle(feature.name, set()):
                logger.error(
                    f"Circular dependency detected for feature '{feature.name}'"
                )
                return False
    ...
    async def _write_to_offline_store(
        ...
        data = {"entity_id": entity_id, "timestamp": timestamp, **features}
    ...
    def _register_default_transformations(self):
        ...
        # Moving average
        self.transformations["moving_avg"] = (
            lambda x, window: pd.Series(x).rolling(window=window or 7).mean()
        )
    ...
    async def _load_feature_definitions(self):
        ...
        default_features = [
            ...
            FeatureDefinition(
                ...
                schema={
                    "type": "string",
                    "enum": ["tech", "gaming", "education", "entertainment"],
                },
            ),
            FeatureDefinition(
                name="trending_score",
    ...
    def get_feature_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "computed_features": sum(
                1 for f in self.features.values() if f.source == FeatureSource.COMPUTED
            ),
            "features_by_type": {
