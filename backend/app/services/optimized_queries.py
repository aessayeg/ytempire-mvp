...
class OptimizedQueryService:
    ...
    @staticmethod
    async def get_channel_with_all_relations(
        ...
        result = await db.execute(
            ...
            .options(
                ...
                selectinload(Channel.videos).options(
                    selectinload(Video.analytics), selectinload(Video.costs)
                ),
            )
    ...
    @staticmethod
    async def get_videos_with_analytics(
        db: AsyncSession, channel_id: int, limit: int = 50
    ) -> List[Video]:
    ...
    async def get_dashboard_data(db: AsyncSession, user_id: int) -> dict:
        ...
        channels_subq = (
            select(Channel.owner_id, func.count(Channel.id).label("channel_count"))
        ...
        costs_subq = (
            select(Channel.owner_id, func.sum(Cost.amount).label("total_cost"))
            .select_from(Channel)
            .join(Video)
            .join(Cost)
            .where(Channel.owner_id == user_id)
        ...
        result = await db.execute(
            ...
            .outerjoin(channels_subq, User.id == channels_subq.c.owner_id)
            .outerjoin(videos_subq, User.id == videos_subq.c.owner_id)
            .outerjoin(costs_subq, User.id == costs_subq.c.owner_id)
            .where(User.id == user_id)
        )
    ...
    @staticmethod
    async def get_recent_videos_optimized(
