...
class InvoiceGenerator:
    ...
    def _setup_custom_styles(self):
        ...
        self.styles.add(
            ParagraphStyle(
                name="InvoiceRight", parent=self.styles["Normal"], alignment=TA_RIGHT
            )
        )
            ...
            items.append(
                {
                    ...
                    "unit_price": usage_charges["video_generation"]
                    / usage_charges["video_count"],
    ...
    async def _calculate_usage_charges(
        self, db, user_id: str, start_date: datetime, end_date: datetime
    ) -> Dict[str, float]:
    ...
    async def _generate_pdf(
        self, invoice: Any, user: Any, subscription: Any, invoice_data: Dict[str, Any]
    ) -> str:
        ...
        items_table = Table(
            items_data, colWidths=[3.5 * inch, 1 * inch, 1.25 * inch, 1.25 * inch]
        )
        ...
        story.append(
            Paragraph(
                "Payment is due within 30 days of invoice date.", self.styles["Normal"]
            )
        )
    ...
    async def mark_invoice_paid(
        self, invoice_id: str, payment_method: str, transaction_id: str
    ) -> bool:
