...
class EnhancedVideoGenerationOrchestrator(VideoGenerationOrchestrator):
    ...
    async def generate_video_with_ml(
        ...
        try:
            ...
            if use_personalization:
                ...
                personalized_content = await get_personalized_video_content(
                    ...
                    channel_data={
                        ...
                        "target_audience": channel.metadata.get("target_audience", {})
                        if channel
                        else {},
                    },
                ...
                logger.info(
                    f"Confidence: {personalized_content['confidence_score']:.2%}"
                )
            ...
            if use_performance_prediction and personalized_content:
                ...
                predicted_performance = await predict_video_performance(
                    video_features, db
                )
                ...
                logger.info(
                    f"Engagement: {predicted_performance['predicted_engagement_rate']:.2%}"
                )

                metrics.phase_durations["performance_prediction"] = (
            ...
            await cost_tracker.track_cost(
                ...
                metadata={
                    "video_id": video_id,
                    "personalized": bool(personalized_content),
                },
            )
            ...
            voice_data = await self.voice_synthesizer.synthesize(
                text=script_data["script"], **voice_settings
            )
            ...
            thumbnail_data = await self.thumbnail_generator.generate(
                title=script_data["title"], keywords=script_data.get("keywords", [])
            )
            ...
            quality_score = await self.quality_scorer.score(
                script=script_data["script"], title=script_data["title"]
            )
            ...
            if use_personalization:
                await update_channel_ml_profile(
                    ...
                    performance_data={
                        ...
                        "predicted_engagement": predicted_performance[
            ...
            result = {
                ...
                "personalization": {
                    ...
                    "confidence": personalized_content["confidence_score"]
                    if personalized_content
                    else 0,
                    "style": personalized_content["style"]
                    if personalized_content
    ...
    async def _get_historical_videos(
        self, channel_id: str, db: AsyncSession, limit: int = 20
    ) -> List[Dict[str, Any]]:
            ...
            return [
                {
                    ...
                    "engagement_rate": ((video.likes or 0) + (video.comments or 0))
                    / max(video.views or 1, 1),
                }
                for video in videos
            ]
        except Exception as e:
    ...
    async def batch_generate_with_ml(
        ...
        if use_personalization and ml_service.automl_pipeline:
            ...
            for channel_id in channel_ids:
                ...
                avg_engagement = insights.get("performance", {}).get(
                    "avg_engagement", 0
                )
...
async def generate_batch_videos_with_ml(
    channel_ids: List[str], topics: Optional[List[str]] = None, db: AsyncSession = None
) -> List[Dict[str, Any]]:
    """Generate multiple videos with ML optimization"""
    return await enhanced_orchestrator.batch_generate_with_ml(
        channel_ids=channel_ids, topics=topics, db=db, use_personalization=True
    )
