...
class TrainingDataService:
    ...
    async def create_dataset(
        ...
        # Store dataset
        storage_path = await self._store_dataset(
            dataset_id, 
            version_id, 
            source_data
        )
        ...
        await self.track_lineage(
            dataset_id=dataset_id,
            source_datasets=[],
            transformations=[{"type": "initial_creation", "timestamp": datetime.utcnow().isoformat()}]
        )
    ...
    async def create_version(
        ...
        # Validate new data
        validation = await self.validate_data(new_data, DatasetType(parent_version.metadata.get("type")))
        if validation.status == ValidationStatus.FAILED:
            raise ValueError(f"Data validation failed: {validation.issues}")
    ...
    async def calculate_statistics(self, data: pd.DataFrame) -> DatasetStatistics:
        ...
        for col in data.select_dtypes(include=[np.number]).columns:
            ...
            IQR = Q3 - Q1
            outliers = data[(data[col] < Q1 - 1.5 * IQR) | (data[col] > Q3 + 1.5 * IQR)][col].tolist()
            if outliers:
        ...
        for col in data.select_dtypes(include=['object', 'category']).columns:
            ...
            statistics.categorical_columns[col] = {
                ...
                "entropy": float(-sum((value_counts / len(data)) * np.log2(value_counts / len(data) + 1e-10)))
            }
    ...
    async def _apply_changes(
        ...
        for transformation in transformations:
            ...
            if transform_type == "append":
                ...
                new_data = pd.merge(new_data, changes, on=merge_on, how=transformation.get("how", "inner"))
    ...
    async def _update_lineage_graph(
        self,
        dataset_id: str,
        source_datasets: List[str]
    ):
