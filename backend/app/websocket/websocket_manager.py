    ...
    async def send_channel_message(self, message: WebSocketMessage, channel_id: str):
        ...
        if self.redis_client:
            await self.redis_client.publish(
                f"ws:channel:{channel_id}", message.to_json()
            )
        ...
        try:
            while True:
                ...
                heartbeat = WebSocketMessage(
                    type=MessageType.HEARTBEAT, data={"status": "ping"}
                )
...
class VideoGenerationNotifier:
    ...
    async def notify_generation_started(
        self, job_id: str, user_id: str, channel_id: str, video_details: Dict[str, Any]
    ):
    ...
    async def notify_generation_progress(
        ...
        message = WebSocketMessage(
            ...
            data={"job_id": job_id, "progress": progress, "current_step": current_step},
            user_id=user_id,
            channel_id=channel_id,
        )
    ...
    async def notify_generation_completed(
        self, job_id: str, user_id: str, channel_id: str, video_data: Dict[str, Any]
    ):
        """Notify that video generation is complete"""
        message = WebSocketMessage(
            type=MessageType.VIDEO_GENERATION_COMPLETED,
            data={"job_id": job_id, "video_data": video_data, "success": True},
            user_id=user_id,
            channel_id=channel_id,
        )
    ...
    async def notify_generation_failed(
        self, job_id: str, user_id: str, channel_id: str, error: str
    ):
        """Notify that video generation failed"""
        message = WebSocketMessage(
            type=MessageType.VIDEO_GENERATION_FAILED,
            data={"job_id": job_id, "error": error, "success": False},
            user_id=user_id,
            channel_id=channel_id,
        )
