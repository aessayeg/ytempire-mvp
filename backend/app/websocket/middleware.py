...
class WebSocketAuthMiddleware:
    ...
    async def authenticate(
        ...
        try:
            ...
            try:
                ...
                if not user_id:
                    await self.reject_connection(websocket, "Invalid token payload")
                    return None
                ...
                await self.reject_connection(websocket, "Token has expired")
                return None

            except jwt.InvalidTokenError as e:
                await self.reject_connection(websocket, f"Invalid token: {str(e)}")
                return None

        except Exception as e:
            logger.error(f"WebSocket authentication error: {str(e)}")
            await self.reject_connection(websocket, "Authentication failed")
            return None

    async def reject_connection(self, websocket: WebSocket, reason: str):
        """
    ...
    async def check_video_permission(
        self, user_id: str, video_id: str, action: str
    ) -> bool:
    ...
    async def check_room_permission(
        self, user_id: str, room_id: str, action: str
    ) -> bool:
