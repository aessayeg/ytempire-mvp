...
class CollaborationHandler(BaseWebSocketHandler):
    ...
    async def handle_connect(
        self, websocket: WebSocket, client_id: str, room_id: Optional[str] = None
    ):
    ...
    async def handle_message(self, client_id: str, message: Dict[str, Any]):
        ...
        if message_type == "join_room":
            ...
            room_id = message.get("room_id")
            if room_id:
                await room_manager.handle_room_message(
                    room_id,
                    client_id,
                    {"type": "typing", "is_typing": message.get("is_typing", False)},
                )
            ...
            if room_id:
                await room_manager.handle_room_message(
                    room_id,
                    client_id,
                    {"type": "cursor_position", "position": message.get("position")},
                )
