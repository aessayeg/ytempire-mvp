...
async def get_template(
    template_id: str, current_user: User = Depends(get_current_verified_user)
) -> Dict[str, Any]:
    """
...
async def list_voice_presets(
    ...
    try:
        ...
        presets = _get_mock_voice_presets(
            str(current_user.id), provider, favorites_only
        )
...
async def test_voice_preset(
    ...
    try:
        ...
        if not preset:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Voice preset not found"
            )
...
async def get_script_suggestions(
    ...
    try:
        ...
        suggestions = _generate_script_suggestions(
            topic, target_duration, style, audience
        )
        ...
        return {
            ...
            "templates_available": len(
                [
                    t
                    for t in _get_mock_templates(str(current_user.id))
                    if t["type"] == "script"
                ]
            ),
        }
...
def _get_mock_voice_presets(
    user_id: str, provider: Optional[VoiceProvider] = None, favorites_only: bool = False
) -> List[Dict[str, Any]]:
...
def _apply_template_variables(
    content: Dict[str, Any], variables: Dict[str, str]
) -> Dict[str, Any]:
    ...
    for key, value in content.items():
        if isinstance(value, str):
            ...
            processed[key] = [
                substitute_text(item) if isinstance(item, str) else item
                for item in value
            ]
        else:
            processed[key] = value
...
def _generate_script_suggestions(
    topic: str, target_duration: int, style: str, audience: str
) -> List[Dict[str, Any]]:
