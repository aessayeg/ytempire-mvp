...
async def get_executive_metrics(
    ...
    try:
        ...
        avg_cost_per_video = total_cost / videos_generated if videos_generated > 0 else 0
        previous_cost_per_video = avg_cost_per_video * 1.1  # Mock 10% improvement
        
        # Quality score
        quality_score = beta_summary.get('average_metrics', {}).get('overall_score', 0) / 10
        ...
        return {
            ...
            "mrr_change_percent": ((mrr - previous_mrr) / previous_mrr * 100) if previous_mrr > 0 else 0,
            
            "arr": arr,
            "previous_arr": previous_arr,
            "arr_change": arr - previous_arr,
            "arr_change_percent": ((arr - previous_arr) / previous_arr * 100) if previous_arr > 0 else 0,
            
            "mau": mau,
            "previous_mau": previous_mau,
            "mau_change": mau - previous_mau,
            "mau_change_percent": ((mau - previous_mau) / previous_mau * 100) if previous_mau > 0 else 0,
            
            "videos_generated": videos_generated,
            "previous_videos": previous_videos,
            "videos_change": videos_generated - previous_videos,
            "videos_change_percent": ((videos_generated - previous_videos) / previous_videos * 100) if previous_videos > 0 else 0,
            ...
            "cost_change_percent": ((avg_cost_per_video - previous_cost_per_video) / previous_cost_per_video * 100) if previous_cost_per_video > 0 else 0,
            
            "quality_score": round(quality_score, 1),
            "previous_quality": round(previous_quality, 1),
            "quality_change": quality_score - previous_quality,
            "quality_change_percent": ((quality_score - previous_quality) / previous_quality * 100) if previous_quality > 0 else 0,
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fetch executive metrics: {str(e)}")
    ...
    try:
        ...
        for kpi_name, kpi_data in kpis.items():
            business_kpis.append({
                ...
                "trend": [kpi_data.current_value * (1 + np.random.normal(0, 0.1)) for _ in range(30)],  # Mock trend data
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch business KPIs: {str(e)}")
...
async def get_financial_performance(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch financial performance: {str(e)}")
...
async def get_competitive_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch competitive analysis: {str(e)}")
...
async def get_business_alerts(
    ...
    try:
        ...
        avg_churn_prob = success_summary.get('average_metrics', {}).get('churn_probability', 0)
        ...
        avg_engagement = success_summary.get('average_metrics', {}).get('overall_score', 0)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch business alerts: {str(e)}")
...
async def get_strategic_insights(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch strategic insights: {str(e)}")
    ...
    try:
        ...
        return {
            ...
            "projections": {
                ...
                "peak_monthly_revenue": max(f["revenue"] for f in forecast) if forecast else 0
            }
        }
...
async def get_cohort_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch cohort analysis: {str(e)}")
...
async def get_conversion_funnels(
    ...
    try:
        ...
        funnels = {
            "signup_to_first_video": {
                ...
                "steps": [
                    ...
                    {"step": "Email Verification", "users": 85, "conversion_rate": 85.0},
                    {"step": "Profile Setup", "users": 72, "conversion_rate": 84.7},
                    {"step": "Channel Connection", "users": 58, "conversion_rate": 80.6},
                    {"step": "First Video Generated", "users": 45, "conversion_rate": 77.6}
                ]
            },
            "trial_to_paid": {
            ...
            "onboarding": {
                ...
                "steps": [
                    ...
                    {"step": "Onboarding Complete", "users": 54, "conversion_rate": 88.5}
                ]
            }
        }
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch conversion funnels: {str(e)}")