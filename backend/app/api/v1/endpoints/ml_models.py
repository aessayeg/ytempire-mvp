...
class ModelConfig(BaseModel):
    ...
    model_type: str = Field(
        ..., description="Model type (trend, script, voice, thumbnail)"
    )
...
async def deploy_model(
    ...
    try:
        ...
        background_tasks.add_task(
            ml_service.start_model_monitoring, deployment_result["model_id"]
        )
...
async def update_model(
    ...
    """
    try:
        result = await ml_service.update_model(model_id=model_id, config=config.dict())
...
async def get_model_metrics(
    ...
    try:
        metrics = await ml_service.get_model_metrics(
            model_id=model_id, timeframe=timeframe
        )
...
async def train_model(
    ...
    try:
        ...
        # Schedule background monitoring
        if background_tasks:
            background_tasks.add_task(ml_service.monitor_training, job_id)
...
async def rollback_model(
    ...
    try:
        ...
        result = await ml_service.rollback_model(model_id=model_id, version=version)
