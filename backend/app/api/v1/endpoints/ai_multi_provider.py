...
class ImageGenerationRequest(BaseModel):
    ...
    provider: Optional[AIProvider] = None
    size: str = Field(
        "1024x1024", regex="^(256x256|512x512|1024x1024|1792x1024|1024x1792)$"
    )
    style: str = Field(
        "realistic", description="Image style: realistic, artistic, cartoon"
    )
...
class SummarizationRequest(BaseModel):
    ...
    style: str = Field(
        "bullet_points", description="Summary style: bullet_points, paragraph, keywords"
    )
    provider: Optional[AIProvider] = None
...
async def analyze_sentiment(
    ...
    try:
        response = await multi_ai_service.analyze_sentiment(
            text=request.text, provider=request.provider, user_id=str(current_user.id)
        )
    ...
    try:
        ...
        return {
            ...
            "processing_details": {
                ...
                "average_latency_ms": sum(r["latency_ms"] for r in results)
                / len(results)
                if results
                else 0,
            },
        ...
        return {
            ...
            "summary": {
                ...
                "available_providers": len(
                    [p for p in status["providers"].values() if p["available"]]
                ),
                "service_coverage": {
...
async def test_provider_connectivity(
    provider_name: str, current_user: User = Depends(get_current_verified_user)
) -> Dict[str, Any]:
    """
        ...
        if service_type:
            ...
            for provider, services in cost_data["cost_comparison"].items():
                if service_type.value in services:
                    filtered_costs[provider] = {
                        service_type.value: services[service_type.value]
                    }
