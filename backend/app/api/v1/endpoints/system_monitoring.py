...
async def get_system_status(
    ...
    try:
        ...
        realtime_status = {
            ...
            'websocket_connections': len(realtime_analytics_service.websocket_connections),
            'buffer_sizes': {
        ...
        return {
            ...
            'beta_success_tracking': {
                ...
                'average_score': beta_summary.get('average_metrics', {}).get('overall_score', 0)
            },
            'system_health': {
                'services_online': 4,
                'total_services': 4,
...
async def get_scaling_info(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get scaling info: {str(e)}")


@router.post("/force-scaling-mode")
async def force_scaling_mode(
    mode: str,
    current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to force scaling mode: {str(e)}")
...
async def get_realtime_dashboard(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get realtime dashboard: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            health_status['services']['scaling_optimizer'] = {
                'status': 'healthy' if scaling_status['processing_active'] else 'unhealthy',
        ...
        if unhealthy_services:
            health_status['overall_status'] = 'degraded' if len(unhealthy_services) < len(health_status['services']) else 'unhealthy'
...
async def get_capacity_planning(
    ...
    try:
        ...
        if not recent_metrics:
            return {
                'error': 'Insufficient data for capacity planning'
            }
        ...
        return {
            ...
            'forecast': forecast,
            'recommendations': {
                'scale_up_at': next((f['time'] for f in forecast if f['required_scaling_mode'] != 'normal'), None),
    ...
    try:
        ...
        return {
            ...
            'service_health_summary': {
                'healthy': len([s for s in dependencies.values() if s['status'] == 'healthy']),
                'degraded': len([s for s in dependencies.values() if s['status'] == 'degraded']),
                'unhealthy': len([s for s in dependencies.values() if s['status'] == 'unhealthy'])
            }
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get service dependencies: {str(e)}")