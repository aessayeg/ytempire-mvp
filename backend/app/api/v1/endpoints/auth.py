...
async def register(
    ...
    if not await rate_limiter.check_rate_limit(
        f"register:{user_data.email}", max_attempts=3, window=3600
    ):
        raise HTTPException(
...
async def login(
    ...
    # Check rate limiting
    if not await rate_limiter.check_rate_limit(
        f"login:{form_data.username}", max_attempts=5, window=900
    ):
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
...
async def verify_email(
    verification: EmailVerification, db: AsyncSession = Depends(get_db)
):
    """
...
async def request_password_reset(
    ...
    # Check rate limiting
    if not await rate_limiter.check_rate_limit(
        f"password_reset:{reset_request.email}", max_attempts=3, window=3600
    ):
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
    ...
    result = await db.execute(select(User).filter(User.email == reset_request.email))
    user = result.scalar_one_or_none()
