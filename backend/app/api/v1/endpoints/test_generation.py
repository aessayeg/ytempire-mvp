...
async def test_video_generation(
    ...
    try:
        ...
        logger.info(
            f"Starting test video generation for channel {channel_id} with topic: {topic}"
        )

        # Generate video
        result = await video_orchestrator.generate_video(
            channel_id=channel_id, topic=topic, db=db
        )
        ...
        validation = {
            ...
            "checks": {
                "generation_time": {
                    ...
                    "actual": result["metrics"]["total_duration_seconds"]
                    if result["success"]
                    else None,
                    "passed": result["metrics"]["total_duration_seconds"] < 600
                    if result["success"]
                    else False,
                },
                "cost": {
                    "target": 3.00,
                    "actual": float(result["metrics"]["total_cost"])
                    if result["success"]
                    else None,
                    "passed": float(result["metrics"]["total_cost"]) < 3.00
                    if result["success"]
                    else False,
                },
                "quality_score": {
                    "target": 70,
                    "actual": result["metrics"]["quality_score"]
                    if result["success"]
                    else None,
                    "passed": result["metrics"]["quality_score"] >= 70
                    if result["success"]
                    else False,
                },
                "youtube_upload": {
                    "target": True,
                    "actual": result.get("youtube_id") is not None
                    if result["success"]
                    else False,
                    "passed": result.get("youtube_id") is not None
                    if result["success"]
                    else False,
                },
            },
        ...
        if validation["p0_requirements_met"]:
            ...
            logger.warning(
                f"⚠️ P0 requirements not fully met. Failed checks: {failed_checks}"
            )
        ...
        return {
            ...
            "summary": {
                ...
                "total_cost_usd": float(result["metrics"]["total_cost"])
                if result["success"]
                else None,
...
async def test_generation_websocket(
    websocket: WebSocket, channel_id: str, db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        # Send final result
        await websocket.send_json({"type": "final_result", "result": result})
...
async def get_generation_status(
    ...
    return {
        ...
        "system_status": {
            ...
            "p0_tasks_ready": True,
            "can_generate_video": quota_status["accounts_available"] > 0
            and float(daily_costs) < 90.00,
        },
    }
...
async def test_batch_generation(
    ...
    if num_videos > 10:
        raise HTTPException(
            status_code=400, detail="Maximum 10 videos for batch generation"
        )
    ...
    results = await video_orchestrator.batch_generate(
        channel_ids=channel_ids, max_concurrent=3, db=db  # Limit concurrent generations
    )
