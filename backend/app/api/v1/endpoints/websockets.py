...
async def video_generation_updates(
    websocket: WebSocket, channel_id: str, token: Optional[str] = None
):
    """
...
async def analytics_stream(websocket: WebSocket, token: Optional[str] = None):
    """
        ...
        async def stream_analytics():
            while True:
                try:
                    ...
                    analytics = await realtime_analytics_service.get_realtime_metrics(
                        user_id
                    )
        ...
        while True:
            try:
                ...
                if message_type == "ping":
                    ...
                    await websocket.send_json(
                        {"type": "filter_applied", "filters": filters}
                    )
...
async def notifications_stream(websocket: WebSocket, token: Optional[str] = None):
    """
        ...
        while True:
            try:
                ...
                if data.get("type") == "ping":
                    ...
                    logger.info(
                        f"Notification {notification_id} acknowledged by {user_id}"
                    )
