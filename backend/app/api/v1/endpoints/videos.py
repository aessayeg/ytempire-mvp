...
class VideoGenerateRequest(BaseModel):
    ...
    style: str = Field(
        "informative",
        description="Video style: informative, entertaining, tutorial, review",
    )
    duration: str = Field(
        "short",
        description="Video duration: short (1-3 min), medium (5-10 min), long (10+ min)",
    )
    voice_style: str = Field(
        "natural", description="Voice style: natural, energetic, calm, professional"
    )
    language: str = Field("en", description="Language code")
    use_trending: bool = Field(True, description="Use trending topics for content")
    quality_preset: str = Field(
        "balanced", description="Quality preset: fast, balanced, quality"
    )
...
# Endpoints
@router.post(
    "/generate", response_model=VideoResponse, status_code=status.HTTP_202_ACCEPTED
)
async def generate_video(
    ...
    result = await db.execute(
        select(func.count(Video.id)).filter(
            Video.channel_id == request.channel_id, Video.created_at >= today_start
        )
    )
    ...
    result = await db.execute(
        select(func.sum(Cost.amount)).filter(
            Cost.user_id == current_user.id, Cost.created_at >= month_start
        )
    )
    ...
    logger.info(
        f"Video generation started: {video.id} for channel {request.channel_id}"
    )
...
async def get_generation_status(
    ...
    result = await db.execute(
        ...
        .join(Channel)
        .filter(Video.id == video_id, Channel.owner_id == current_user.id)
    )
    row = result.first()
...
async def list_videos(
    ...
    query = (
        ...
        .options(selectinload(Video.channel), selectinload(Video.costs))
    )
...
async def bulk_generate_videos(
    ...
    if not topics:
        trending = await ai_service.get_trending_topics(
            channel.category, count=request.count
        )
    ...
    for i in range(request.count):
        video = Video(
            ...
            generation_status="pending",
            publish_status="draft",
            scheduled_publish_time=start_date
            + timedelta(hours=i * request.schedule_interval_hours),
            created_at=datetime.utcnow(),
        )
    ...
    return {
        ...
        "scheduled_times": [
            (
                start_date + timedelta(hours=i * request.schedule_interval_hours)
            ).isoformat()
            for i in range(request.count)
        ],
    }
...
async def get_video_analytics(
    ...
    profit_margin = (
        (profit / video.actual_revenue * 100) if video.actual_revenue > 0 else 0
    )
    ...
    return {
        ...
        "performance": {
            ...
            "average_view_duration": round(
                video.watch_time_minutes * 60 / max(video.view_count, 1), 2
            ),
        },
        "financial": {
            "total_cost": video.total_cost,
            ...
            "roi": round(
                (profit / video.total_cost * 100) if video.total_cost > 0 else 0, 2
            ),
        },
        "quality": {
            "quality_score": video.quality_score,
