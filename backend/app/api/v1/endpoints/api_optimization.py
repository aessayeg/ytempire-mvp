...
async def get_provider_configs(current_user: User = Depends(get_current_verified_user)):
    ...
    try:
        ...
        for config_key, config in api_optimizer.api_configs.items():
            configs[config_key] = {
                ...
                "fallback_providers": [
                    fp.value for fp in (config.fallback_providers or [])
                ],
        ...
        return {
            ...
            "configs": configs,
            "total_providers": len(
                set(config.provider for config in api_optimizer.api_configs.values())
            ),
        }
...
async def reset_circuit_breaker(
    provider: APIProvider, current_user: User = Depends(get_current_verified_user)
):
    ...
    try:
        ...
        return {
            ...
            "test_result": {
                ...
                "circuit_breaker_open": api_optimizer._is_circuit_breaker_open(
                    provider
                ),
