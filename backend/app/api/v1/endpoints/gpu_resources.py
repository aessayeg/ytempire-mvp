...
class GPUAllocationRequest(BaseModel):
    ...
    task_id: str
    task_type: str = Field(
        ..., description="Type of task: video_generation, model_training, inference"
    )
    memory_required: int = Field(..., ge=1, description="Required GPU memory in bytes")
    priority: int = Field(
        default=1, ge=0, le=3, description="Task priority (0=low, 3=critical)"
    )
...
async def allocate_gpu(
    ...
    try:
        ...
        if device_id is not None:
            ...
            queue_position = (
                len([t for t in queue if t["priority"] >= request.priority]) + 1
            )
    ...
    try:
        ...
        return [
            GPUDeviceStatus(
                ...
                memory_utilization=(device["memory_used"] / device["memory_total"])
                * 100,
                gpu_utilization=device["utilization_gpu"],
                temperature=device["temperature"],
                power_draw=device["power_draw"],
                reserved_by=device["reserved_by"],
                reserved_until=datetime.fromisoformat(device["reserved_until"])
    ...
    try:
        ...
        for i, task in enumerate(queue):
            task_info = GPUTaskInfo(
                ...
                allocated_at=datetime.fromisoformat(task["allocated_at"])
...
async def get_gpu_statistics(
    ...
    try:
        ...
        avg_utilization = sum(d["utilization_gpu"] for d in devices) / max(
            total_devices, 1
        )
...
async def reserve_gpu(
    ...
    duration_minutes: int = Query(
        60, ge=1, le=480, description="Reservation duration in minutes"
    ),
    ...
    try:
        ...
        reservation_id = (
            f"res_{current_user.id}_{device_id}_{int(datetime.utcnow().timestamp())}"
        )
...
@router.get("/health")
async def gpu_health_check(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    """
...
async def get_efficiency_analysis(
    time_window_hours: int = Query(
        24, ge=1, le=168, description="Time window for analysis"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_verified_user),
) -> Dict[str, Any]:
    ...
    try:
        ...
        total_devices = len(devices)
        active_devices = len(
            [d for d in devices if d["status"] in ["busy", "reserved"]]
        )

        if total_devices > 0:
            device_utilization = active_devices / total_devices * 100
            avg_gpu_utilization = (
                sum(d["utilization_gpu"] for d in devices) / total_devices
            )
            avg_memory_utilization = (
                sum((d["memory_used"] / d["memory_total"]) * 100 for d in devices)
                / total_devices
            )
        else:
        ...
        if metrics:
            recent_efficiency = sum(m["efficiency_score"] for m in metrics[-10:]) / min(
                10, len(metrics)
            )
            older_efficiency = sum(m["efficiency_score"] for m in metrics[:10]) / min(
                10, len(metrics)
            )
        ...
        memory_fragmented = [
            d
            for d in devices
            if (d["memory_total"] - d["memory_used"]) > (2 * 1024 * 1024 * 1024)
        ]
        ...
        return {
            ...
            "current_efficiency": {
                ...
                "overall_efficiency_score": round(
                    (device_utilization + avg_gpu_utilization + avg_memory_utilization)
                    / 3,
                    1,
                ),
            },
...
async def balance_workload(
    target_utilization: float = Query(
        75.0, ge=50.0, le=95.0, description="Target GPU utilization percentage"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_verified_user),
) -> Dict[str, Any]:
    ...
    try:
        ...
        if load_analysis["overloaded_devices"] and load_analysis["underloaded_devices"]:
            for overloaded in load_analysis["overloaded_devices"]:
                for underloaded in load_analysis["underloaded_devices"]:
                    if (
                        underloaded["available_capacity"] > 10
                    ):  # Only if significant capacity
                        rebalancing_plan.append(
                            {
                                "action": "migrate_tasks",
                                "from_device": overloaded["device_id"],
                                "to_device": underloaded["device_id"],
                                "estimated_tasks_to_move": min(
                                    2, int(overloaded["excess_load"] / 20)
                                ),
                                "expected_improvement": min(
                                    15, overloaded["excess_load"] / 2
                                ),
                            }
                        )
                        break
        ...
        return {
            ...
            "estimated_improvement": {
                "balance_improvement": sum(
                    plan["expected_improvement"] for plan in rebalancing_plan
                ),
                "efficiency_gain_percent": min(20, len(rebalancing_plan) * 5),
                "tasks_optimized": sum(
                    plan["estimated_tasks_to_move"] for plan in rebalancing_plan
                ),
            },
            "timestamp": datetime.utcnow().isoformat(),
        }
