...
async def train_performance_model(
    ...
    try:
        ...
        result = await training_service.train_video_performance_model(
            db=db, min_videos=request.min_videos, force_retrain=request.force_retrain
        )
...
async def train_quality_model(
    ...
    try:
        result = await training_service.train_content_quality_model(
            db=db, min_videos=request.min_videos
        )
...
async def trigger_retraining(
    request: TriggerRetrainingRequest, current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def monitor_model(
    request: MonitorModelRequest, current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        result = await training_service.monitor_model_performance(
            model_name=request.model_name, threshold=request.threshold
        )
...
async def get_training_status(
    ...
    try:
        ...
        return {"pipeline_available": False, "status": "error", "message": str(e)}
