...
class BatchVideoGenerationRequest(BaseModel):
    ...
    scheduled_at: Optional[datetime] = Field(
        None, description="Schedule job for future execution"
    )
    dependencies: Optional[List[str]] = Field(
        None, description="Job IDs that must complete first"
    )
    checkpoint_interval: int = Field(
        default=5, description="Save progress every N videos"
    )
...
async def batch_generate_videos(
    ...
    try:
            ...
            video_requests.append(
                {
                    ...
                    "scheduled_publish_time": video_req.scheduled_publish_time.isoformat()
                    if video_req.scheduled_publish_time
                    else None,
        ...
        for i, req in enumerate(video_requests):
            items.append(BatchJobItem(id=f"{job_id}_item_{i}", data=req))
        ...
        job_id = await batch_processor.submit_batch_job(
            job_config, video_batch_processor._process_video_generation
        )

        # Calculate estimated completion time
        estimated_time = (
            len(request.videos) * 5
        ) / request.max_concurrent  # 5 min per video
...
async def get_job_status(
    ...
    try:
        ...
        return {"success": True, "job": status_data}
...
async def validate_batch_video_request(
    ...
    try:
        ...
        if len(request.videos) > 10 and request.max_concurrent > 5:
            warnings.append(
                "High concurrency with many videos may impact system performance"
            )
        ...
        for i, video in enumerate(request.videos):
            ...
            if len(video.tags) > 10:
                warnings.append(
                    f"Video {i+1}: More than 10 tags may impact performance"
                )
...
async def get_batch_templates(current_user: User = Depends(get_current_verified_user)):
...
@router.get("/metrics")
async def get_batch_metrics(current_user: User = Depends(get_current_verified_user)):
...
async def create_chained_jobs(
    jobs: List[Dict[str, Any]], current_user: User = Depends(get_current_verified_user)
):
