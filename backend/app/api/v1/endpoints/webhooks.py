...
async def test_webhook(
    ...
    test_payload = test_request.sample_data or generate_sample_payload(test_request.event)
    
    # Send webhook
    background_tasks.add_task(
        deliver_webhook,
        webhook,
        test_request.event.value,
        test_payload,
        is_test=True
    )
...
async def toggle_webhook(
    ...
    webhook['status'] = WebhookStatus.ACTIVE if webhook['active'] else WebhookStatus.INACTIVE
...
async def trigger_event(
    ...
    for webhook in webhooks_storage.values():
        if (webhook['user_id'] == str(current_user.id) and 
            webhook['active'] and 
...
def generate_sample_payload(event: WebhookEvent) -> Dict[str, Any]:
    ...
    return sample_payloads.get(event, {"event": event.value, "timestamp": datetime.utcnow().isoformat()})