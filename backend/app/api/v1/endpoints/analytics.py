    ...
    try:
        ...
        return {
            "query": {
                ...
                "aggregation_level": query.aggregation_level.value
                if query.aggregation_level
                else None,
            },
            "results": results,
...
async def get_channel_roi(
    ...
    try:
        channel_roi = await roi_calculator.calculate_channel_roi(
            db=db, channel_id=channel_id, period_days=period_days
        )
...
async def get_video_roi(
    ...
    """
    try:
        video_roi = await roi_calculator.calculate_video_roi(db=db, video_id=video_id)
