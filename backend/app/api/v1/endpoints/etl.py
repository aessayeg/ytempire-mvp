...
async def run_video_performance_etl(
    ...
    try:
        result = await etl_service.run_video_performance_etl(
            db=db, incremental=request.incremental
        )
...
async def run_channel_analytics_etl(
    ...
    try:
        result = await etl_service.run_channel_analytics_etl(
            db=db, channel_id=request.channel_id
        )
...
async def schedule_etl_pipeline(
    request: ScheduleETLRequest, current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        result = await etl_service.schedule_etl_pipeline(
            pipeline_name=request.pipeline_name, cron_expression=request.cron_expression
        )
...
async def get_data_quality_report(
    dimension: Optional[str] = None, current_user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
