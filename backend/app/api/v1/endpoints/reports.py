...
class ReportRequest(BaseModel):
    ...
    report_type: str = Field(..., description="Type of report (performance, cost, analytics, summary)")
    ...
    try:
        ...
        if request.report_type == "performance":
            report_data = await _generate_performance_report(
                db, current_user.id, request.start_date, request.end_date,
                request.channel_ids, request.include_details
            )
        ...
        if request.format == "csv":
            ...
            filename = f"report_{request.report_type}_{datetime.now().strftime('%Y%m%d')}.csv"
            ...
            pdf_filename = f"report_{request.report_type}_{datetime.now().strftime('%Y%m%d')}.pdf"
...
async def _generate_performance_report(
    ...
    if include_details:
        for video in videos[:50]:  # Limit to top 50
            video_metrics.append({
                ...
                "engagement_rate": (video.like_count / video.view_count * 100) if video.view_count else 0,
    ...
    if channel_ids:
        for channel_id in channel_ids:
            ...
            channel_metrics.append({
                ...
                "avg_views": sum(v.view_count or 0 for v in channel_videos) / len(channel_videos) if channel_videos else 0
            })
    ...
    return {
        ...
        "summary": {
            ...
            "avg_views_per_video": round(total_views / total_videos, 0) if total_videos else 0
        },
...
async def _generate_summary_report(
    ...
    return {
        ...
        "costs": {
            ...
            "per_video": costs["cost_per_video"]
        },
        "roi": {
            "revenue_estimate": performance["summary"]["total_views"] * 0.002,  # $2 CPM estimate
            "profit_margin": ((performance["summary"]["total_views"] * 0.002) - costs["total_cost"]) / costs["total_cost"] * 100 if costs["total_cost"] else 0
        },
        "highlights": [
