...
async def get_revenue_overview(
    start_date: Optional[datetime] = Query(
        None, description="Start date for revenue period"
    ),
    end_date: Optional[datetime] = Query(
        None, description="End date for revenue period"
    ),
...
async def get_channel_revenue(
    ...
    try:
        ...
        if not revenue_data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Revenue data not found"
            )
...
async def get_revenue_trends(
    ...
    try:
        ...
        trends = await revenue_tracking_service.get_revenue_trends(
            db=db, user_id=current_user.id, period=period, lookback_days=lookback_days
        )
...
async def get_revenue_forecast(
    ...
    try:
        ...
        forecast = await revenue_tracking_service.get_revenue_forecast(
            db=db, user_id=current_user.id, forecast_days=forecast_days
        )
...
async def get_revenue_breakdown(
    breakdown_by: str = Query(
        "source",
        description="Breakdown dimension: source, content_type, video_length, time_of_day",
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_verified_user),
) -> RevenueBreakdown:
    ...
    try:
        ...
        breakdown = await revenue_tracking_service.get_revenue_breakdown(
            db=db, user_id=current_user.id, breakdown_by=breakdown_by
        )
    ...
    try:
        ...
        if format == "csv":
            ...
            for channel in overview["channel_breakdown"]:
                writer.writerow([channel["channel_name"], channel["revenue"]])
...
async def recalculate_revenue(
    channel_id: Optional[int] = Query(
        None, description="Specific channel to recalculate"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_verified_user),
):
