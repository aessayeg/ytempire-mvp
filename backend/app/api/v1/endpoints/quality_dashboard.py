...
async def get_quality_overview(current_user=Depends(get_current_verified_user)):
...
async def record_metric(
    metric_request: MetricValueRequest, current_user=Depends(get_current_verified_user)
):
...
async def get_defect_statistics(current_user=Depends(get_current_verified_user)):
...
async def get_defect(defect_id: str, current_user=Depends(get_current_verified_user)):
...
async def get_available_reports(current_user=Depends(get_current_verified_user)):
...
async def generate_report(
    ...
    try:
        ...
        reporting_system = setup_automated_reporting(
            metrics_collector, quality_dashboard, defect_tracker
        )

        # Generate report based on type
        if report_type == "daily_summary":
            report_data = await reporting_system.generator.generate_daily_summary(
                datetime.now()
            )
        elif report_type == "weekly_quality":
            week_start = datetime.now() - timedelta(days=7)
            report_data = (
                await reporting_system.generator.generate_weekly_quality_report(
                    week_start
                )
            )
            ...
            report_data = (
                await reporting_system.generator.generate_monthly_executive_report(
                    month_start
                )
            )
        else:
        ...
        if format == "html" and report_type in reporting_system.generator.templates:
            ...
            html_content = jinja_template.render(
                css_styles=template.css_styles, **report_data.get("data", {})
            )
    ...
    try:
        ...
        if latest_metrics:
            ...
            data_freshness_minutes = (
                datetime.now(timezone.utc) - newest_metric_time
            ).total_seconds() / 60
        else:
        ...
        return {"status": "error", "message": str(e), "data": {"status": "unhealthy"}}
...
async def start_monitoring(current_user=Depends(get_current_verified_user)):
...
async def stop_monitoring(current_user=Depends(get_current_verified_user)):
