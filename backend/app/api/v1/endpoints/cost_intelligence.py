...
class OptimizationRequest(BaseModel):
    ...
    services_to_optimize: List[str] = Field(
        default=[], description="Specific services to optimize"
    )
...
class BudgetManagementRequest(BaseModel):
    ...
    auto_actions_enabled: bool = Field(
        True, description="Enable automatic cost control actions"
    )


@router.get("/predict")
async def predict_costs(
    timeframe: PredictionTimeframe = Query(
        PredictionTimeframe.MONTH, description="Prediction timeframe"
    ),
    ...
    try:
        ...
        predictions = await cost_optimizer.predict_future_costs(
            days_ahead=days_ahead, include_confidence=include_confidence
        )

        if "error" in predictions:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=predictions["error"]
            )
    ...
    try:
        ...
        return {
            ...
            "optimization_analysis": {
                ...
                "expected_outcomes": {
                    "estimated_monthly_savings": filtered_recommendations.get(
                    ...
                    "implementation_effort": _assess_implementation_effort(
                        filtered_recommendations
                    ),
                },
            },
...
async def setup_budget_management(
    ...
    try:
        ...
        budget_analysis = await cost_optimizer.implement_budget_management(
            monthly_budget=monthly_budget, alert_thresholds=request.alert_thresholds
        )

        if "error" in budget_analysis:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=budget_analysis["error"]
            )
...
def _filter_recommendations_by_level(
    recommendations: Dict[str, Any], level: OptimizationLevel, quality_threshold: float
) -> Dict[str, Any]:
    """Filter recommendations based on optimization level and quality threshold"""
    filtered = {"recommendations": [], "estimated_total_savings": 0}
        ...
        for rec in recommendations.get("recommendations", []):
            ...
            if rec.get("quality_impact", 100) >= quality_threshold:
                if rec.get("savings_percent", 0) <= max_reduction * 100:
                    ...
                    filtered["estimated_total_savings"] += rec.get(
                        "estimated_savings", 0
                    )
...
def _generate_implementation_roadmap(recommendations: Dict[str, Any]) -> Dict[str, Any]:
    ...
    roadmap = {"phases": [], "total_duration_days": 0, "priority_order": []}
        ...
        roadmap["phases"].append(
            {
                ...
                "expected_savings": sum(
                    r.get("estimated_savings", 0) for r in quick_wins
                ),
            }
        )
        ...
        roadmap["phases"].append(
            {
                ...
                "expected_savings": sum(
                    r.get("estimated_savings", 0) for r in medium_effort
                ),
            }
        )
        ...
        roadmap["phases"].append(
            {
                ...
                "expected_savings": sum(
                    r.get("estimated_savings", 0) for r in high_effort
                ),
            }
        )
...
def _generate_budget_insights(
    budget_analysis: Dict[str, Any], request: BudgetManagementRequest
) -> Dict[str, Any]:
    ...
    if "current_spend" in budget_analysis and request.budget_amount > 0:
        ...
        if utilization > 90:
            ...
            insights["recommendations"].append(
                "Consider implementing cost optimization"
            )
        else:
