...
from app.services.payment_service_enhanced import (
    payment_service,
    PaymentService,
    SubscriptionStatus,
)
...
class UsageTrackingRequest(BaseModel):
    ...
    usage_type: str = Field(
        ..., description="Type of usage: videos, api_calls, storage"
    )
...
class UpgradeDowngradeRequest(BaseModel):
    ...
    proration_behavior: str = Field(
        default="create_prorations", description="How to handle proration"
    )
...
async def stripe_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await payment_service.process_webhook(
            db=db, payload=payload, signature=signature
        )
            ...
            plan = {
                ...
                "limits": {
                    ...
                    "api_calls_monthly": int(
                        product.metadata.get("api_calls_monthly", 5000)
                    ),
                    "channels": int(product.metadata.get("channels", 3)),
                    "video_length_minutes": int(
                        product.metadata.get("video_length_minutes", 10)
                    ),
                },
            }
...
async def cancel_subscription(
    ...
    try:
        ...
        subscription = stripe.Subscription.modify(
            subscription_data["subscription_id"], cancel_at_period_end=True
        )
...
async def resume_subscription(
    ...
    try:
        ...
        # Resume subscription
        subscription = stripe.Subscription.modify(
            subscription_data["subscription_id"], cancel_at_period_end=False
        )
...
async def remove_payment_method(
    ...
    try:
        result = await payment_service.remove_payment_method(
            db=db, user_id=str(current_user.id), payment_method_id=payment_method_id
        )
                ...
                plans.append(
                    {
                        ...
                        "recommended": price.product.metadata.get(
                            "recommended", "false"
                        )
                        == "true",
                    }
