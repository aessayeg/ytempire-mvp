...
sys.path.append(
    os.path.join(
        os.path.dirname(__file__), "..", "..", "..", "..", "..", "ml-pipeline", "src"
    )
)
...
class ScriptGenerationRequest(BaseModel):
    ...
    duration_minutes: int = Field(
        default=5, ge=1, le=30, description="Target video duration in minutes"
    )
    ...
    include_hook: bool = Field(
        default=True, description="Include attention-grabbing hook"
    )
    include_cta: bool = Field(default=True, description="Include call-to-action")
    quality_preset: str = Field(
        default="balanced", description="Quality preset: fast, balanced, or quality"
    )
    trending_context: Optional[Dict[str, Any]] = Field(
        default=None, description="Trending data context"
    )
    channel_context: Optional[Dict[str, Any]] = Field(
        default=None, description="Channel style context"
    )
...
async def generate_script(
    ...
    try:
        ...
        script_response = await script_generator.generate_script(
            script_request, quality_preset=request.quality_preset
        )
        ...
        background_tasks.add_task(
            store_script_in_db, db, script_id, current_user.id, script_response, request
        )
...
async def generate_script_variations(
    ...
    try:
        ...
        variations = await script_generator.generate_variations(
            base_request, num_variations=request.num_variations
        )
        ...
        await cost_service.track_cost(
            ...
            cost=total_cost,
            metadata={"topic": request.topic, "num_variations": len(variations)},
        )
...
async def optimize_script(
    ...
    try:
        ...
        await cost_service.track_cost(
            ...
            metadata={"script_id": request.script_id},
        )
...
async def get_script_from_db(
    db: AsyncSession, script_id: str, user_id: str
) -> Optional[Dict]:
    """Retrieve script from database"""
    # This is a placeholder - implement actual DB retrieval
