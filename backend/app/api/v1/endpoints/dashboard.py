    ...
    try:
        ...
        video_stats = await db.execute(
            select(
                ...
                func.sum(
                    func.case((Video.created_at >= today_start, 1), else_=0)
                ).label("today"),
                func.sum(func.case((Video.created_at >= week_start, 1), else_=0)).label(
                    "week"
                ),
                func.sum(
                    func.case((Video.created_at >= month_start, 1), else_=0)
                ).label("month"),
            ).where(
        ...
        cost_stats = await db.execute(
            ...
            ).where(Cost.user_id == current_user.id)
        )
        ...
        avg_performance = (
            (video_data.views / video_data.total) if video_data.total > 0 else 0
        )
...
async def get_performance_metrics(
    ...
    try:
        ...
        analytics_data = await db.execute(
            ...
            .where(
                and_(Analytics.user_id == current_user.id, Analytics.date >= start_date)
            )
    ...
    try:
        ...
        result = await db.execute(
            select(
                ...
                and_(
                    Analytics.user_id == current_user.id, Analytics.date >= last_30_days
                )
            )
        )
...
async def get_realtime_stats(
    ...
    try:
        ...
        today_cost = await db.execute(
            select(func.sum(Cost.amount)).where(
                and_(Cost.user_id == current_user.id, Cost.created_at >= today)
            )
        )
    ...
    notifications = [
        {
            ...
            "read": False,
            "priority": "normal",
            "metadata": {"video_id": "video_123", "channel_name": "Tech Channel"},
        },
        {
            "id": "notif_2",
            "type": "milestone",
            ...
            "read": True,
            "priority": "high",
            "metadata": {"milestone_amount": 100, "currency": "USD"},
        },
    ]
...
async def mark_notification_read(
    notification_id: str, current_user: User = Depends(get_current_verified_user)
) -> Dict[str, str]:
    """
...
def calculate_channel_performance(
    video_count: int, total_views: int, subscriber_count: int
) -> float:
