...
class VideoProcessingRequest(BaseModel):
    ...
    enable_quality_enhancement: bool = Field(False, description="Enable AI quality enhancement")
    ...
    enable_gpu_acceleration: bool = Field(True, description="Use GPU acceleration if available")
...
class MultiFormatExportRequest(BaseModel):
    ...
    enable_parallel_processing: bool = Field(True, description="Process formats in parallel")


class BatchVideoProcessingRequest(BaseModel):
    """Request for batch video processing"""
    videos: List[Dict[str, Any]] = Field(..., description="List of video processing jobs")
...
class QualityEnhancementRequest(BaseModel):
    ...
    enhancement_type: str = Field("auto", description="Enhancement type: auto, denoising, sharpening, color")
...
async def process_video(
    ...
    try:
        ...
        config = VideoProcessingConfig(
            ...
            resolution=tuple(request.resolution) if request.resolution else (1920, 1080),
        ...
        if not request.output_path:
            ...
            request.output_path = str(output_dir / f"processed_{uuid.uuid4().hex[:8]}.{request.format.value}")
...
async def batch_process_videos(
    ...
    try:
        ...
        config = VideoProcessingConfig(
            ...
            resolution=tuple(request.config.resolution) if request.config.resolution else (1920, 1080),
...
async def cleanup_processing_files(
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Cleaned up files for job {job_id}"
        }
