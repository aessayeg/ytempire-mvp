...
class CacheManager:
    ...
    def __init__(
        ...
        self.cache_stats = {"hits": 0, "misses": 0, "errors": 0, "evictions": 0}
...
class ConnectionPoolManager:
    ...
    async def create_http_pool(self) -> aiohttp.ClientSession:
        ...
        timeout = aiohttp.ClientTimeout(
            total=30, connect=5, sock_connect=5, sock_read=10
        )

        connector = aiohttp.TCPConnector(
            limit=100, limit_per_host=30, ttl_dns_cache=300, enable_cleanup_closed=True
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if response_time > 500:  # Slow request threshold
                ...
                logger.warning(
                    f"Slow request: {request.url} took {response_time:.2f}ms"
                )
                ...
                if isinstance(response, StreamingResponse):
                    ...
                    try:
                        ...
                        await self.cache_manager.set(
                            cache_key, orjson.loads(body), ttl=60
                        )
                    except Exception as e:
