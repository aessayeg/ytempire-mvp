...
def batch_process_videos(
    ...
    try:
        logger.info(
            f"Starting batch processing for job {batch_job_id} with {len(videos)} videos"
        )
...
@celery_app.task(
    base=BatchTask, name="batch.process_single_video", queue="batch_processing"
)
def process_single_video(
    ...
    try:
        ...
        # Update item status
        asyncio.run(update_batch_item_status(batch_job_id, item_index, "processing"))
        ...
        asyncio.run(
            update_batch_item_status(
                batch_job_id, item_index, "failed", {"error": str(e)}
            )
        )

        return {"success": False, "item_index": item_index, "error": str(e)}


@celery_app.task(name="batch.bulk_update", queue="batch_processing")
def bulk_update_videos(
    video_ids: List[str], updates: Dict[str, Any], user_id: str = None
) -> Dict[str, Any]:
    """
...
@celery_app.task(name="batch.bulk_delete", queue="batch_processing")
def bulk_delete_videos(
    video_ids: List[str], user_id: str = None, soft_delete: bool = True
) -> Dict[str, Any]:
    """
    Bulk delete videos
...
@celery_app.task(name="batch.analytics_aggregation", queue="batch_processing")
def batch_aggregate_analytics(
    channel_ids: List[str], start_date: str, end_date: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        tasks = [calculate_channel_roi.s(channel_id, 30) for channel_id in channel_ids]
        ...
        # Aggregate results
        total_revenue = sum(
            r["metrics"]["revenue"] for r in results if r.get("success")
        )
...
async def update_batch_status(
    batch_job_id: str, status: BatchJobStatus, metadata: Dict[str, Any] = None
):
...
async def update_batch_item_status(
    batch_job_id: str, item_index: int, status: str, result: Dict[str, Any] = None
):
...
# Cleanup task
@celery_app.task(name="batch.cleanup_old_jobs", queue="batch_processing")
        ...
        async def cleanup():
            async with AsyncSessionLocal() as db:
                ...
                result = await db.execute(
                    select(BatchJob).where(BatchJob.created_at < cutoff_date)
                )
