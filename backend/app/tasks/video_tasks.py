...
class VideoTaskBase(Task):
        ...
        if video_id:
            with next(get_db()) as db:
                ...
                if video:
                    ...
                    websocket_manager.send_video_update(
                        video_id, {"status": "failed", "error": str(exc)}
                    )
    ...
    def on_retry(self, exc, task_id, args, kwargs, einfo):
        ...
        if video_id:
            websocket_manager.send_video_update(
                video_id, {"status": "retrying", "attempt": self.request.retries + 1}
            )
...
def generate_video(
    ...
    try:
        ...
        with next(get_db()) as db:
            ...
            websocket_manager.send_video_update(
                video_id, {"status": "processing", "step": "initializing"}
            )
            ...
            websocket_manager.send_video_update(
                video_id,
                {"status": "processing", "step": "generating_script", "progress": 10},
            )
            ...
            websocket_manager.send_video_update(
                video_id,
                {"status": "processing", "step": "generating_voice", "progress": 30},
            )
            ...
            websocket_manager.send_video_update(
                video_id,
                {"status": "processing", "step": "generating_visuals", "progress": 50},
            )
            ...
            websocket_manager.send_video_update(
                video_id,
                {"status": "processing", "step": "creating_video", "progress": 70},
            )
            ...
            websocket_manager.send_video_update(
                video_id,
                {"status": "processing", "step": "quality_check", "progress": 85},
            )
            ...
            if video.auto_upload:
                upload_to_youtube.apply_async(
                    kwargs={"video_id": video_id}, priority=TaskPriority.NORMAL
                )
...
def upload_to_youtube(self, video_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        with next(get_db()) as db:
            ...
            # Send WebSocket update
            websocket_manager.send_video_update(
                video_id, {"status": "uploading", "platform": "youtube"}
            )
...
@celery_app.task(
    name="app.tasks.video_tasks.cleanup_old_videos", priority=TaskPriority.LOW
)
...
@celery_app.task(
    name="app.tasks.video_tasks.check_stuck_videos", priority=TaskPriority.LOW
)
    ...
    try:
        ...
        with next(get_db()) as db:
            ...
            stuck_videos = (
                db.query(Video)
                .filter(
                    Video.status == VideoStatus.PROCESSING, Video.started_at < timeout
                )
                .all()
            )
            ...
            for video in stuck_videos:
                ...
                # Send notification
                websocket_manager.send_video_update(
                    video.id, {"status": "failed", "error": "Processing timeout"}
                )
...
def batch_generate_videos(
    self, channel_id: int, topics: list, style: str = "educational"
):
    ...
    try:
        ...
        for topic in topics:
            ...
            with next(get_db()) as db:
                video = Video(
                    channel_id=channel_id, title=topic, status=VideoStatus.QUEUED
                )
                db.add(video)
                db.commit()
                db.refresh(video)
                ...
                results.append(
                    {"video_id": video.id, "task_id": task.id, "topic": topic}
                )
