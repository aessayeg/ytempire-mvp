        ...
        async def _deliver():
            async with get_db() as db:
                success = await webhook_service.deliver_webhook(
                    db=db, delivery_id=uuid.UUID(delivery_id)
                )
                return success
    ...
    try:
        ...
        async def _process_batch():
            async with get_db() as db:
                ...
                logger.info(
                    f"Processed {processed_count}/{len(event_data_list)} webhook events"
                )
                    ...
                    if stats.total > 0:
                        ...
                        if success_rate < 0.5 and webhook.failure_count > 10:
                            ...
                            webhook.last_error_message = (
                                f"Suspended due to low success rate: {success_rate:.2%}"
                            )
...
def setup_periodic_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(
        3600.0, update_webhook_stats.s(), name="update_webhook_stats"  # 1 hour
    )
