...
class DDoSDetector:
    ...
    async def check_request(self, request: Request) -> Tuple[bool, Optional[str], Optional[Dict]]:
        """
        ...
        if not await self._check_connection_limit(client_ip):
            return False, "connection_limit", {"ip": client_ip, "limit": DDoSConfig.MAX_CONNECTIONS_PER_IP}
        
        # Check rate limits
        rate_check = await self._check_rate_limits(client_ip)
        if not rate_check[0]:
            return False, f"rate_limit_{rate_check[1]}", {"ip": client_ip, "limit_type": rate_check[1]}
        ...
        if pattern:
            return False, f"attack_pattern_{pattern}", {"ip": client_ip, "pattern": pattern}
    ...
    async def _check_connection_limit(self, ip: str) -> bool:
        ...
        if self.connection_count[ip] > DDoSConfig.MAX_CONNECTIONS_PER_IP:
            logger.warning(f"Connection limit exceeded for {ip}: {self.connection_count[ip]}")
            return False
    ...
    async def _detect_burst(self, ip: str) -> bool:
        ...
        burst_requests = [t for t in history if current_time - t < DDoSConfig.BURST_WINDOW]
        
        if len(burst_requests) > DDoSConfig.BURST_THRESHOLD:
            logger.warning(f"Burst detected from {ip}: {len(burst_requests)} requests in {DDoSConfig.BURST_WINDOW}s")
            return True
    ...
    async def _issue_challenge(self, ip: str, request: Request) -> Tuple[bool, str, Dict]:
...
class DDoSProtectionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            is_allowed, block_reason, metadata = await self.detector.check_request(request)
            ...
            if hasattr(response, "headers"):
                response.headers["X-RateLimit-Limit"] = str(DDoSConfig.MAX_REQUESTS_PER_MINUTE)
                response.headers["X-RateLimit-Remaining"] = str(
                    DDoSConfig.MAX_REQUESTS_PER_MINUTE - len(self.detector.request_history.get(client_ip, []))
                )
