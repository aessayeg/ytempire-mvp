...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        is_allowed, retry_after = await self._check_rate_limit(
            client_id, rate_config["max_requests"], rate_config["window_seconds"]
        )
    ...
    async def _get_rate_config(self, request: Request) -> dict:
        ...
        # Default limits
        default_config = {"max_requests": 100, "window_seconds": 60}
        ...
        if client_id.startswith("user:"):
            ...
            default_config["max_requests"] = int(
                default_config["max_requests"] * multiplier
            )
...
async def require_api_key(request: Request, api_key: str = None) -> dict:
    """
