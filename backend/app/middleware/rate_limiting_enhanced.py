...
class EnhancedRateLimiter:
    ...
    async def check_rate_limit(
        ...
        if "videos/generate" in endpoint:
            checks.append(
                self._check_daily_video_limit(
                    client_id, limits["video_generation_per_day"]
                )
            )
        ...
        headers = {
            ...
            "X-RateLimit-Remaining": str(
                min(r[1].get("remaining", 0) for r in results)
            ),
    ...
    async def _check_window(
        self, client_id: str, window: str, duration: int, limit: int
    ) -> Tuple[bool, Dict]:
    ...
    async def _check_burst(self, client_id: str, burst_size: int) -> Tuple[bool, Dict]:
        ...
        tokens_to_add = time_passed * (
            burst_size / 3600
        )  # Refill rate: burst_size per hour
    ...
    async def _check_daily_video_limit(
        self, client_id: str, daily_limit: int
    ) -> Tuple[bool, Dict]:
    ...
    def _apply_endpoint_limits(self, base_limits: Dict, endpoint_config: Dict) -> Dict:
        ...
        modified_limits["requests_per_minute"] = max(
            1, modified_limits["requests_per_minute"] // multiplier
        )
        modified_limits["requests_per_hour"] = max(
            1, modified_limits["requests_per_hour"] // multiplier
        )
        ...
        return {
            ...
            "usage": {
                ...
                "videos_today": {
                    ...
                    "percentage": (video_count / limits["video_generation_per_day"])
                    * 100,
                },
            },
        }
