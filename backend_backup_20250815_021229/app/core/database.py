...
is_sqlite = not hasattr(settings, 'DATABASE_URL') or 'sqlite' in str(settings.DATABASE_URL)

# Create async engine with proper pooling
if is_sqlite:
    # SQLite doesn't support connection pooling well
    engine = create_async_engine(
        settings.DATABASE_URL if hasattr(settings, 'DATABASE_URL') else "sqlite+aiosqlite:///./ytempire.db",
        echo=False,
