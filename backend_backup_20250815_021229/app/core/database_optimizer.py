...
class DatabaseOptimizer:
    ...
    async def set_cached(self, query: str, params: Dict[str, Any], result: Any, ttl: int = 300):
        """Cache query result"""
        key = self.cache_key(query, params)
    ...
    async def execute_prepared(self, conn: asyncpg.Connection, name: str, query: str, *args):
    ...
    async def batch_insert(self, session: AsyncSession, table: str, records: List[Dict]):
        ...
        """), {f"{col}_{i}": val for i, record in enumerate(records) for col, val in record.items()})
    ...
    async def auto_optimize(self, session: AsyncSession):
        ...
        if slow_queries:
            logger.warning(f"Found {len(slow_queries)} slow queries requiring optimization")
            for query in slow_queries[:5]:
                logger.info(f"Slow query (mean: {query['mean_time']:.2f}ms): {query['query'][:100]}...")
