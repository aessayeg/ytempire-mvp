...
class SecretVault:
    ...
    async def retrieve_secret(self, secret_id: str) -> Optional[str]:
        ...
        try:
            # Try Redis first
            if self.redis_client:
                encrypted_value = await self.redis_client.get(f"secret:{secret_id}:value")
                if encrypted_value:
...
class SecretsRotationManager:
    ...
    def __init__(
        ...
        self.rotation_policies: Dict[SecretType, RotationPolicy] = self._load_default_policies()
    ...
    async def rotate_secret(
        ...
        try:
            ...
            if success:
                # Archive old secret
                if old_secret:
                    await self._archive_old_secret(secret_id, old_secret, metadata.version - 1)
                
                # Update dependent services
                await self._update_dependent_services(secret_id, secret_type, new_secret)
            ...
            if self.db_session:
                await self.db_session.execute(
                    ...
                    """),
                    {
                        "secret_id": secret_id,
                        "version": version,
                        "encrypted_value": self.vault.fernet.encrypt(old_secret.encode()).decode()
                    }
                )
                await self.db_session.commit()
        except Exception as e:
    ...
    async def _update_dependent_services(self, secret_id: str, secret_type: SecretType, new_secret: str):
    ...
    async def check_expiring_secrets(self) -> List[Dict]:
        ...
        if self.db_session:
            for secret_type, policy in self.rotation_policies.items():
                notify_date = datetime.now(timezone.utc) + timedelta(days=policy.notify_before_days)
                ...
                for row in result:
                    expiring.append({
                        ...
                        "days_until_expiry": (row.expires_at - datetime.now(timezone.utc)).days
                    })
            ...
            try:
                ...
                for secret in expiring:
                    if secret["days_until_expiry"] <= 0:
                        ...
                        # Send notification
                        logger.warning(f"Secret {secret['secret_id']} expires in {secret['days_until_expiry']} days")
