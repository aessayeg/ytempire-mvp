...
class BetaSuccessMetricsService:
    ...
    def _define_success_stages(self) -> Dict[SuccessStage, Dict[str, Any]]:
        ...
        return {
            ...
            SuccessStage.ADVANCED_USE: {
                ...
                'expected_actions': ['manage_multiple_channels', 'automated_publishing']
            },
    ...
    async def _load_user_profile(self, user_id: str) -> Optional[UserSuccessProfile]:
        ...
        try:
            ...
            if cached_data:
                ...
                profile_data['signup_date'] = datetime.fromisoformat(profile_data['signup_date'])
                ...
                if profile_data.get('last_video_generated'):
                    profile_data['last_video_generated'] = datetime.fromisoformat(profile_data['last_video_generated'])
                
                profile_data['current_stage'] = SuccessStage(profile_data['current_stage'])
    ...
    async def _calculate_user_metrics(self, user_id: str, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            onboarding_steps = ['profile_setup', 'channel_connect', 'first_video', 'first_publish']
            completed_steps = set(e.get('event_type') for e in events)
            metrics['onboarding_completion'] = (len(completed_steps & set(onboarding_steps)) / len(onboarding_steps)) * 100
            
            # Time to first video
            signup_time = user_data.get('signup_date')
            first_video_event = next((e for e in events if e.get('event_type') == 'video_generate'), None)
            
            if signup_time and first_video_event:
                signup_dt = datetime.fromisoformat(signup_time) if isinstance(signup_time, str) else signup_time
                first_video_dt = datetime.fromisoformat(first_video_event['timestamp'])
                metrics['time_to_first_video'] = int((first_video_dt - signup_dt).total_seconds() / 60)
            
            # Video generation metrics
            metrics['videos_generated_total'] = len(videos)
            
            video_events_7d = [e for e in events if e.get('event_type') == 'video_generate' 
                             and datetime.fromisoformat(e['timestamp']) > seven_days_ago]
            metrics['videos_generated_7d'] = len(video_events_7d)
            
            video_events_24h = [e for e in events if e.get('event_type') == 'video_generate' 
            ...
            core_features = {'video_generate', 'analytics_view', 'channel_manage', 'cost_tracking', 'auto_publish'}
            metrics['feature_adoption_score'] = (len(features_used & core_features) / len(core_features)) * 100
            
            # Quality metrics
            if videos:
                performance_scores = [v.get('performance_score', 0) for v in videos if v.get('performance_score')]
                metrics['avg_video_performance'] = np.mean(performance_scores) if performance_scores else 0.0
                
                published_videos = len([v for v in videos if v.get('status') == 'published'])
                metrics['published_video_ratio'] = published_videos / len(videos) if videos else 0.0
            else:
            ...
            revenue_events = [e for e in events if e.get('event_type') == 'revenue_earned']
            
            total_cost = sum(e.get('data', {}).get('amount', 0) for e in cost_events)
            total_revenue = sum(e.get('data', {}).get('amount', 0) for e in revenue_events)
            
            metrics['cost_efficiency'] = total_revenue / total_cost if total_cost > 0 else 0.0
            
            # Support and engagement
            support_events = [e for e in events if e.get('event_type') == 'support_ticket']
            ...
            community_events = [e for e in events if e.get('event_type') in ['community_post', 'community_comment']]
            metrics['community_engagement'] = len(community_events)
            
            referral_events = [e for e in events if e.get('event_type') == 'referral_made']
    ...
    def _calculate_overall_score(self, metrics: Dict[str, Any]) -> float:
        ...
        for metric_name, metric_def in self.success_metrics.items():
            ...
            if metric_def.measurement_type == 'time':
                ...
                normalized_score = max(0, min(100, (metric_def.target_value - value) / metric_def.target_value * 100))
            else:
    ...
    def _determine_success_stage(self, metrics: Dict[str, Any]) -> SuccessStage:
        ...
        for stage in [SuccessStage.ADVOCATE, SuccessStage.ADVANCED_USE, SuccessStage.REGULAR_USE,
                     SuccessStage.FIRST_USE, SuccessStage.ONBOARDING, SuccessStage.SIGNUP]:
    ...
    def _calculate_risk_level(self, metrics: Dict[str, Any]) -> RiskLevel:
        ...
        for risk_level in [RiskLevel.LOW, RiskLevel.MEDIUM, RiskLevel.HIGH, RiskLevel.CRITICAL]:
            ...
            if (overall_score >= thresholds['score_min'] and 
                active_days >= thresholds['activity_days'] and 
                videos_7d >= thresholds['videos_7d']):
                return risk_level
    ...
    def _calculate_churn_probability(self, metrics: Dict[str, Any]) -> float:
        ...
        factors = [
            ...
            min(1.0, metrics.get('videos_generated_7d', 0) / 7),  # Video generation rate
    ...
    def _predict_lifetime_value(self, metrics: Dict[str, Any]) -> float:
        ...
        ltv = monthly_videos * 10 * quality_factor * engagement_factor * 12  # Annual LTV
    ...
    def _predict_next_action(self, metrics: Dict[str, Any], events: List[Dict]) -> str:
        ...
        recent_events = [e for e in events 
                        if datetime.fromisoformat(e['timestamp']) > datetime.utcnow() - timedelta(days=2)]
    ...
    async def _cache_user_profile(self, profile: UserSuccessProfile):
        ...
        if profile.last_video_generated:
            profile_data['last_video_generated'] = profile.last_video_generated.isoformat()
    ...
    async def _success_monitor(self):
        ...
        while True:
            try:
                ...
                successful_users = [
                    profile for profile in self.beta_users.values()
                    if profile.overall_score > 85 and profile.current_stage == SuccessStage.ADVOCATE
                ]
    ...
    async def _generate_success_alerts(self, successful_users: List[UserSuccessProfile]):
        ...
        for user in successful_users:
            alert = {
                ...
                'overall_score': user.overall_score,
                'potential_actions': ['request_testimonial', 'invite_to_case_study', 'referral_program'],
                'timestamp': datetime.utcnow().isoformat()
            }
            
            alert_key = f"alerts:success:milestone:{datetime.utcnow().strftime('%Y%m%d%H')}"
            await self.redis_client.lpush(alert_key, json.dumps(alert))
            await self.redis_client.expire(alert_key, 86400)
    ...
    async def _calculate_kpis(self):
        ...
        activated_users = len([p for p in profiles 
                              if p.time_to_first_video and p.time_to_first_video <= 24 * 60])
        self.kpi_definitions['activation_rate'].current_value = (activated_users / len(profiles)) * 100
        
        # 7-Day Retention
        week_ago = now - timedelta(days=7)
        retained_users = len([p for p in profiles 
                             if p.last_activity > week_ago])
        self.kpi_definitions['retention_7d'].current_value = (retained_users / len(profiles)) * 100
        
        # Daily Engagement Rate
        daily_active = len([p for p in profiles if p.daily_active_days_7d >= 1])
        self.kpi_definitions['engagement_rate'].current_value = (daily_active / len(profiles)) * 100
        
        # Feature Adoption Rate
        avg_feature_adoption = np.mean([p.feature_adoption_score for p in profiles])
        self.kpi_definitions['feature_adoption_rate'].current_value = avg_feature_adoption
        
        # Quality Threshold
        quality_users = len([p for p in profiles if p.avg_video_performance >= 7.0])
        self.kpi_definitions['quality_threshold'].current_value = (quality_users / len(profiles)) * 100
        
        # Cost Efficiency
        avg_cost_efficiency = np.mean([p.cost_efficiency for p in profiles if p.cost_efficiency > 0])
        ...
        while True:
            try:
                ...
                for profile in self.beta_users.values():
                    ...
                    # Onboarding stalled
                    if (profile.current_stage == SuccessStage.ONBOARDING and 
                        (datetime.utcnow() - profile.signup_date).days > 1):
    ...
    async def _trigger_interventions(self, profile: UserSuccessProfile, interventions: List[str]):
        ...
        await self.redis_client.setex(intervention_key, 86400 * 7, json.dumps(intervention_data))
    ...
    async def get_user_success_profile(self, user_id: str) -> Optional[UserSuccessProfile]:
    ...
    async def get_success_summary(self) -> Dict[str, Any]:
        ...
        avg_metrics = {
            ...
            'onboarding_completion': np.mean([p.onboarding_completion for p in profiles]),
        ...
        return {
            ...
            'stage_distribution': {stage.value: count for stage, count in stage_counts.items()},
