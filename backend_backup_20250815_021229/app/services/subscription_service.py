    ...
    def __init__(self):
        ...
        self.plans = {
            ...
            'enterprise': {
                ...
                'features': ['Everything in Scale', 'Custom integrations', 'SLA', 'Training']
            }
        }
    ...
    async def _create_stripe_subscription(
        ...
        try:
            ...
            subscription = stripe.Subscription.create(
                customer=customer.id,
                items=[{
                    'price': settings.STRIPE_PRICE_IDS.get(plan_id)
                }],
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if new_plan['price'] <= old_plan['price']:
                    return {
                        'success': False,
                        'error': 'New plan must be higher tier'
                    }
    ...
    async def check_usage_limits(
        ...
        async with AsyncSessionLocal() as db:
            ...
            return {
                ...
                'can_create_video': sub.video_limit < 0 or videos_used < sub.video_limit,
                'channels_used': channels_used,
                'channel_limit': sub.channel_limit if sub.channel_limit > 0 else 'Unlimited',
                'can_add_channel': sub.channel_limit < 0 or channels_used < sub.channel_limit,
            ...
            async with AsyncSessionLocal() as db:
                ...
                for subscription in due_subscriptions.scalars():
                    try:
                        ...
                        logger.error(f"Billing cycle processing failed for subscription {subscription.id}: {str(e)}")
    ...
    async def get_subscription_status(
        ...
        async with AsyncSessionLocal() as db:
            ...
            payments = await db.execute(
                ...
                ).order_by(PaymentHistory.created_at.desc()).limit(5)
            )
