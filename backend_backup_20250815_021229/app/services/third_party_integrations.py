...
class ThirdPartyIntegrationService:
    ...
    async def _register_default_integrations(self):
        ...
        if hasattr(settings, 'SLACK_WEBHOOK_URL'):
            await self.register_integration(IntegrationConfig(
                name="slack",
        ...
        if hasattr(settings, 'AIRTABLE_API_KEY'):
            await self.register_integration(IntegrationConfig(
                ...
                custom_settings={
                    ...
                    "table_name": getattr(settings, 'AIRTABLE_TABLE_NAME', 'Videos')
                }
            ))
    ...
    async def call_api(
        ...
        try:
            ...
            logger.error(f"API call failed: {integration_name} {method} {endpoint}: {e}")
            raise
    ...
    async def send_webhook(
        ...
        if config.webhook_events and "all" not in config.webhook_events:
            if event_type not in config.webhook_events:
                logger.debug(f"Event {event_type} not configured for {integration_name}")
                return
        
        # Create webhook event
        event = WebhookEvent(
            id=hashlib.md5(f"{integration_name}{event_type}{datetime.now()}".encode()).hexdigest(),
        ...
        try:
            ...
            logger.error(f"Failed to send webhook: {integration_name} - {event_type}: {e}")
    ...
    async def handle_incoming_webhook(
        ...
        # Find handlers
        handlers = []
        handlers.extend(self.webhook_handlers.get(f"{integration_name}:{event_type}", []))
        ...
        if not handlers:
            logger.warning(f"No handlers for webhook: {integration_name} - {event_type}")
            return
    ...
    async def subscribe_to_mailchimp(
        ...
        result = await self.call_api(
            "mailchimp",
            "PUT",
            f"/lists/{list_id}/members/{member_id}",
            data=data
        )
