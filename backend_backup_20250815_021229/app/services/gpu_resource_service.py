...
class EnhancedGPUResourceService:
    ...
    async def optimize_gpu_allocation(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            current_efficiency = self._calculate_system_efficiency(device_status, current_tasks)
            
            # Batch compatible tasks
            batched_tasks = self._batch_compatible_tasks(current_tasks)
            
            # Redistribute allocations
            optimized_allocation = self._optimize_task_distribution(batched_tasks, device_status)
            
            # Calculate potential efficiency improvement
            potential_efficiency = self._calculate_potential_efficiency(optimized_allocation, device_status)
            
            improvement_percentage = ((potential_efficiency - current_efficiency) / current_efficiency * 100) if current_efficiency > 0 else 0
            ...
            return {
                ...
                "recommendations": self._generate_optimization_recommendations(device_status, current_tasks)
            }
    ...
    async def enable_task_batching(self, db: AsyncSession, task_types: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for batch in batches:
                if len(batch) > 1:  # Only batch if multiple tasks
                    ...
                    # Find suitable device
                    device_id = await self._find_suitable_device(db, total_memory, max(task.priority for task in batch))
                    ...
                    if device_id is not None:
                        ...
                        scheduled_batches.append({
                            ...
                            "estimated_efficiency_gain": min(30.0, len(batch) * 8.0)  # Up to 30% gain
                        })
            ...
            return {
                ...
                "tasks_batched": sum(batch["task_count"] for batch in scheduled_batches),
                "scheduled_batches": scheduled_batches,
                "estimated_total_efficiency_gain": sum(batch["estimated_efficiency_gain"] for batch in scheduled_batches) / len(scheduled_batches) if scheduled_batches else 0
            }
    ...
    async def implement_smart_memory_management(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            for device in devices:
                ...
                result = await db.execute(
                    select(GPUTaskModel).where(
                        and_(
                            ...
                            GPUTaskModel.status.in_([TaskStatus.ALLOCATED, TaskStatus.RUNNING])
                        )
                    )
                )
                ...
                if device_tasks:
                    ...
                    memory_analysis = self._analyze_memory_patterns(device, device_tasks)
                    
                    # Implement dynamic memory allocation
                    if memory_analysis["can_optimize"]:
                        optimization = await self._optimize_device_memory(db, device_id, device_tasks, memory_analysis)
                        
                        if optimization["memory_saved"] > 0:
                            memory_optimizations.append({
                                "device_id": device_id,
                                "memory_saved_mb": optimization["memory_saved"] / (1024 * 1024),
                                "tasks_optimized": len(optimization["optimized_tasks"]),
            ...
            return {
                ...
                "efficiency_improvement": min(30.0, (total_memory_saved / (8 * 1024 * 1024 * 1024)) * 20)  # Estimate based on memory saved
            }
    ...
    def _calculate_system_efficiency(self, devices: List[Dict], tasks: List[Dict]) -> float:
        ...
        for device in devices:
            ...
            memory_score = (device["memory_used"] / device["memory_total"]) * 0.3  # 30% weight
    ...
    def _optimize_task_distribution(self, batched_tasks: List[List[Dict]], devices: List[Dict]) -> Dict[str, Any]:
        ...
        for i, batch in enumerate(batched_tasks):
            if available_devices:
                ...
                allocation["device_assignments"][device_id].append({
                    "batch_id": f"batch_{i}",
                    "task_count": len(batch),
                    "memory_required": sum(task.get("memory_required", 0) for task in batch)
                })
        
        # Calculate efficiency scores
        allocation["load_balance_score"] = self._calculate_load_balance_score(allocation["device_assignments"])
        allocation["memory_efficiency"] = self._calculate_memory_efficiency(allocation["device_assignments"], devices)
        
        return allocation
    
    def _calculate_potential_efficiency(self, allocation: Dict, devices: List[Dict]) -> float:
        ...
        batch_count = sum(len(batches) for batches in allocation["device_assignments"].values())
    ...
    def _generate_optimization_recommendations(self, devices: List[Dict], tasks: List[Dict]) -> List[str]:
        ...
        if underutilized:
            recommendations.append(f"Consider consolidating tasks on {len(underutilized)} underutilized devices")
        
        # Check for memory inefficiency
        inefficient_memory = [d for d in devices if d["memory_used"] / d["memory_total"] < 0.5 and d["utilization_gpu"] > 70]
        if inefficient_memory:
            recommendations.append(f"Optimize memory allocation on {len(inefficient_memory)} devices with low memory usage")
        
        # Check for batching opportunities
        pending_tasks = [t for t in tasks if t["status"] == "pending"]
        if len(pending_tasks) > 3:
            recommendations.append("Enable task batching to improve throughput by up to 30%")
        
        # Temperature warnings
        hot_devices = [d for d in devices if d["temperature"] > 80]
        if hot_devices:
            recommendations.append(f"Monitor thermal performance on {len(hot_devices)} devices running hot")
        
        return recommendations
    
    def _create_task_batches(self, tasks: List[GPUTaskModel]) -> List[List[GPUTaskModel]]:
        ...
        for task_type, group_tasks in task_groups.items():
            ...
            for task in group_tasks:
                if (current_memory + task.memory_required <= max_batch_memory and 
                    len(current_batch) < 4):  # Max 4 tasks per batch
    ...
    def _analyze_memory_patterns(self, device: Dict, tasks: List[GPUTaskModel]) -> Dict[str, Any]:
        ...
        fragmentation_ratio = (total_allocated - current_used) / total_allocated if total_allocated > 0 else 0
        
        can_optimize = (
            fragmentation_ratio > 0.2 or  # More than 20% fragmentation
            (total_available - total_allocated) > (1024 * 1024 * 1024)  # More than 1GB unused
        )
        ...
        return {
            ...
            "optimization_potential": min(30, fragmentation_ratio * 50 + (total_available - total_allocated) / (1024 * 1024 * 1024) * 5)
        }
    
    async def _optimize_device_memory(self, db: AsyncSession, device_id: int, tasks: List[GPUTaskModel], analysis: Dict) -> Dict[str, Any]:
        ...
        for task in tasks:
            ...
            if analysis["fragmentation_ratio"] > 0.2:
                ...
                task.memory_required = max(task.memory_required - reduction, task.memory_required // 2)
    ...
    def _calculate_memory_efficiency(self, assignments: Dict, devices: List[Dict]) -> float:
        ...
        for device in devices:
            ...
            if device_id in assignments:
                ...
                if utilization <= 0.8:
                    ...
                    efficiency = max(0, 1 - (utilization - 0.8) * 5)  # Penalty for over-utilization
