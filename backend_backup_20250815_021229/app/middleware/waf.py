...
class WAFRules:
    ...
    DANGEROUS_FILE_EXTENSIONS = {
        '.exe', '.dll', '.scr', '.bat', '.cmd', '.com', '.pif',  # Windows executables
        '.sh', '.bash', '.zsh',  # Unix scripts
        '.app', '.deb', '.rpm',  # Application packages
        '.jsp', '.asp', '.aspx', '.php', '.cgi',  # Server scripts
        '.swf', '.jar',  # Java/Flash
    }
...
class WAFEngine:
    ...
    def _compile_patterns(self) -> Dict[str, List[re.Pattern]]:
        ...
        return {
            'sql_injection': [re.compile(p, re.IGNORECASE) for p in WAFRules.SQL_INJECTION_PATTERNS],
            'xss': [re.compile(p, re.IGNORECASE) for p in WAFRules.XSS_PATTERNS],
            'path_traversal': [re.compile(p, re.IGNORECASE) for p in WAFRules.PATH_TRAVERSAL_PATTERNS],
            'command_injection': [re.compile(p, re.IGNORECASE) for p in WAFRules.COMMAND_INJECTION_PATTERNS],
            'xxe': [re.compile(p, re.IGNORECASE) for p in WAFRules.XXE_PATTERNS],
            'ldap_injection': [re.compile(p, re.IGNORECASE) for p in WAFRules.LDAP_INJECTION_PATTERNS],
            'nosql_injection': [re.compile(p, re.IGNORECASE) for p in WAFRules.NOSQL_INJECTION_PATTERNS],
            'user_agent': [re.compile(p, re.IGNORECASE) for p in WAFRules.SUSPICIOUS_USER_AGENTS],
        }
    
    async def check_request(self, request: Request) -> Tuple[bool, Optional[str], Optional[str]]:
        """
            ...
            if header in headers:
                ...
                for pattern in self.compiled_patterns['xss']:
                    if pattern.search(value):
                        return "header_injection", f"Potential header injection in {header}"
        
        # Check for HTTP response splitting
        for name, value in headers.items():
            if '\r' in value or '\n' in value:
                return "response_splitting", f"HTTP response splitting attempt in header {name}"
...
class WAFMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            is_safe, threat_type, threat_details = await self.waf_engine.check_request(request)
