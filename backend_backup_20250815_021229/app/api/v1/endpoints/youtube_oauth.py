...
async def list_authorized_accounts(
    current_user = Depends(get_current_user)
):
    """
...
async def generate_all_auth_urls(
    current_user = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        return {
            "total_unauthorized": len(auth_urls),
            "accounts": auth_urls
        }
        
    except Exception as e:
        logger.error(f"Failed to generate auth URLs: {e}")
        raise HTTPException(status_code=500, detail="Failed to generate authorization URLs")