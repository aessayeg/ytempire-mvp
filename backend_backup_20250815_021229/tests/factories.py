...
class UserFactory(factory.Factory):
    ...
    is_active = True
    is_verified = factory.LazyAttribute(lambda o: random.choice([True, True, True, False]))
    is_admin = factory.LazyAttribute(lambda o: random.choice([False, False, False, True]))
...
class ChannelFactory(factory.Factory):
    ...
    category = factory.LazyAttribute(
        lambda o: random.choice([
            ...
            'Music', 'Sports', 'News', 'Comedy', 'Science', 'Travel'
        ])
    )
...
class VideoFactory(factory.Factory):
    ...
    estimated_revenue = factory.LazyAttribute(lambda o: round(random.uniform(0, 1000), 2))
    
    # Scheduling
    scheduled_date = factory.LazyAttribute(
        lambda o: fake.future_datetime(end_date='+30d') if o.status == 'scheduled' else None
    )
    uploaded_at = factory.LazyAttribute(
        lambda o: fake.past_datetime(start_date='-7d') if o.status == 'uploaded' else None
    )
    ...
    language = factory.LazyAttribute(lambda o: random.choice(['en', 'es', 'fr', 'de', 'ja']))
...
class CostFactory(factory.Factory):
    ...
    operation = factory.LazyAttribute(
        lambda o: random.choice([
            'script-generation', 'voice-synthesis', 'thumbnail-generation',
            'video-processing', 'translation', 'transcription'
        ])
    )
    ...
    unit_cost = factory.LazyAttribute(lambda o: o.amount / o.units if o.units > 0 else 0)
    tokens_used = factory.LazyAttribute(
        lambda o: {
            'input': random.randint(100, 5000),
            'output': random.randint(100, 2000)
        } if 'gpt' in o.service_name else None
    )
...
class AnalyticsFactory(factory.Factory):
    ...
    likes = factory.LazyAttribute(lambda o: random.randint(0, o.views // 10 if o.views > 0 else 0))
    dislikes = factory.LazyAttribute(lambda o: random.randint(0, o.views // 100 if o.views > 0 else 0))
    comments = factory.LazyAttribute(lambda o: random.randint(0, o.views // 20 if o.views > 0 else 0))
    shares = factory.LazyAttribute(lambda o: random.randint(0, o.views // 50 if o.views > 0 else 0))
